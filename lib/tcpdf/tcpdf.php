...
class TCPDF {
	...
	public function GetArrStringWidth($sa, $fontname='', $fontstyle='', $fontsize=0, $getarray=false) {
		...
		$w = 0; 		$wa = array(); 		foreach ($sa as $ck => $char) {
	...
	protected function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		$rs = ''; 		$this->adjustCellPadding($border);
		if (!$ignore_min_height) {
			$min_cell_height = $this->getCellHeight($this->FontSize);
			if ($h < $min_cell_height) {
				$h = $min_cell_height;
		...
		if ($txt != '') {
			...
			if ($this->isunicode) {
				if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
					...
					$unicode = TCPDF_FONTS::UTF8StringToArray($txt, $this->isunicode, $this->CurrentFont); 					$unicode = TCPDF_FONTS::utf8Bidi($unicode, '', $this->tmprtl, $this->isunicode, $this->CurrentFont);
			...
			if (isset($uniblock)) {
								$xshift = 0; 				$ty = (($this->h - $basefonty + (0.2 * $this->FontSize)) * $k);
				...
				foreach ($uniblock as $uk => $uniarr) {
					if (($uk % 2) == 0) {
						...
						$xshift += $this->GetArrStringWidth($uniarr); 					} else {
	...
	protected function getCellBorder($x, $y, $w, $h, $brd) {
		...
		foreach ($brd as $border => $style) {
			...
			switch ($mode) {
				...
				if (strpos($border,'B') === false) { 					$s .= sprintf('%F %F m ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= 'S ';
				} elseif (strpos($border,'L') === false) { 					$s .= sprintf('%F %F m ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= 'S ';
				} elseif (strpos($border,'T') === false) { 					$s .= sprintf('%F %F m ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= 'S ';
				} elseif (strpos($border,'R') === false) { 					$s .= sprintf('%F %F m ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= 'S ';
				...
				if ((strpos($border,'L') !== false) AND (strpos($border,'T') !== false)) { 					$s .= sprintf('%F %F m ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= 'S ';
				} elseif ((strpos($border,'T') !== false) AND (strpos($border,'R') !== false)) { 					$s .= sprintf('%F %F m ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= 'S ';
				} elseif ((strpos($border,'R') !== false) AND (strpos($border,'B') !== false)) { 					$s .= sprintf('%F %F m ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= 'S ';
				} elseif ((strpos($border,'B') !== false) AND (strpos($border,'L') !== false)) { 					$s .= sprintf('%F %F m ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= 'S ';
				} elseif ((strpos($border,'L') !== false) AND (strpos($border,'R') !== false)) { 					$s .= sprintf('%F %F m ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= 'S ';
					$s .= sprintf('%F %F m ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= 'S ';
				} elseif ((strpos($border,'T') !== false) AND (strpos($border,'B') !== false)) { 					$s .= sprintf('%F %F m ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= 'S ';
			...
			} else { 				if (strpos($border,'L') !== false) { 					$s .= sprintf('%F %F m ', $xL, $yL);
					$s .= sprintf('%F %F l ', $xT, $yT);
					$s .= 'S ';
				} elseif (strpos($border,'T') !== false) { 					$s .= sprintf('%F %F m ', $xT, $yT);
					$s .= sprintf('%F %F l ', $xR, $yR);
					$s .= 'S ';
				} elseif (strpos($border,'R') !== false) { 					$s .= sprintf('%F %F m ', $xR, $yR);
					$s .= sprintf('%F %F l ', $xB, $yB);
					$s .= 'S ';
				} elseif (strpos($border,'B') !== false) { 					$s .= sprintf('%F %F m ', $xB, $yB);
					$s .= sprintf('%F %F l ', $xL, $yL);
					$s .= 'S ';
				}
			}
	...
	public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign='T', $fitcell=false) {
		...
		if ($ishtml) { 			$this->writeHTML($txt, true, false, $reseth, true, $align);
			$nl = 1;
		} else { 			$prev_FontSizePt = $this->FontSizePt;
			if ($fitcell) {
				...
				if ($fitcell AND ($text_height > $maxh) AND ($this->FontSizePt > 1)) {
					...
					$diff_epsilon = (1 / $this->k); 					$maxit = (2 * min(100, max(10, intval($fmax)))); 					while ($maxit >= 0) {
						$fmid = (($fmax + $fmin) / 2);
		...
		$border_middle = TCPDF_STATIC::getBorderMode($border, $position='middle', $this->opencell);
				for ($page = $startpage; $page <= $endpage; ++$page) { 			$ccode = '';
			$this->setPage($page);
			...
			if ($startpage == $endpage) {
								for ($column = $startcolumn; $column <= $endcolumn; ++$column) { 					$this->selectColumn($column);
					if ($this->rtl) {
						$this->x -= $mc_margin['R'];
					} else {
						$this->x += $mc_margin['L'];
					}
					if ($startcolumn == $endcolumn) { 						$cborder = $border;
						$h = max($h, ($currentY - $oy));
						$this->y = $oy;
					} elseif ($column == $startcolumn) { 						$cborder = $border_start;
						$this->y = $oy;
						$h = $this->h - $this->y - $this->bMargin;
					} elseif ($column == $endcolumn) { 						$cborder = $border_end;
						$h = $currentY - $this->y;
						if ($resth > $h) {
							$h = $resth;
						}
					} else { 						$cborder = $border_middle;
						$h = $this->h - $this->y - $this->bMargin;
						$resth -= $h;
					}
					$ccode .= $this->getCellCode($w, $h, '', $cborder, 1, '', $fill, '', 0, true)."\n";
				} 			} elseif ($page == $startpage) { 				for ($column = $startcolumn; $column < $this->num_columns; ++$column) { 					$this->selectColumn($column);
					if ($this->rtl) {
						$this->x -= $mc_margin['R'];
					} else {
						$this->x += $mc_margin['L'];
					}
					if ($column == $startcolumn) { 						$cborder = $border_start;
						$this->y = $oy;
						$h = $this->h - $this->y - $this->bMargin;
					} else { 						$cborder = $border_middle;
						$h = $this->h - $this->y - $this->bMargin;
					...
					$ccode .= $this->getCellCode($w, $h, '', $cborder, 1, '', $fill, '', 0, true)."\n";
				} 			} elseif ($page == $endpage) { 				for ($column = 0; $column <= $endcolumn; ++$column) { 					$this->selectColumn($column);
					if ($this->rtl) {
						$this->x -= $mc_margin['R'];
					} else {
						$this->x += $mc_margin['L'];
					...
					$ccode .= $this->getCellCode($w, $h, '', $cborder, 1, '', $fill, '', 0, true)."\n";
				} 			} else { 				for ($column = 0; $column < $this->num_columns; ++$column) { 					$this->selectColumn($column);
					if ($this->rtl) {
						$this->x -= $mc_margin['R'];
					} else {
						$this->x += $mc_margin['L'];
	...
	public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		if ($margin === '') {
			...
			$rtlmode = false;
				...
				$chrwidth = $this->GetCharWidth(46); 				$chars = TCPDF_FONTS::UTF8StringToArray($s, $this->isunicode, $this->CurrentFont);
		...
		$i = 0; 		$j = 0; 		$sep = -1; 		$prevsep = $sep; 		$shy = false; 		$prevshy = $shy; 		$l = 0; 		$nl = 0; 		$linebreak = false;
		$pc = 0; 				while ($i < $nb) {
		...
		$parse_error = '';
			...
			try {
				...
				$img->readImage($file);
					...
					$img->separateImageChannel(8); 					$img->negateImage(true);
				}
		...
		switch($dest) {
			case 'I': {
				...
				if (php_sapi_name() != 'cli') {
					...
					header('Expires: Sat, 26 Jul 1997 05:00:00 GMT'); 					header('Last-Modified: '.gmdate('D, d M Y H:i:s').' GMT');
	...
	protected function _putpages() {
		...
		for ($n = 1; $n <= $num_pages; ++$n) {
			...
			$pdiff = 0; 			$gdiff = 0; 			if (!empty($this->pagegroups)) {
							...
							if (is_string($pl['txt'])) {
								if ($pl['txt'][0] == '#') {
									...
									if (empty($parsedUrl['scheme']) AND (strtolower(substr($parsedUrl['path'], -4)) == '.pdf')) {
																				$dest = '[0 /Fit]'; 										if (!empty($parsedUrl['fragment'])) {
			...
			if (!TCPDF_STATIC::empty_string($fontfile)) {
				...
				if ((!$compressed) AND (isset($info['length2']))) {
					...
					if ($compressed) {
										...
										$subsetchars = array(); 					foreach ($info['fontkeys'] as $fontkey) {
	...
	protected function _puttruetypeunicode($font) {
		...
		if (isset($font['ctg']) AND (!TCPDF_STATIC::empty_string($font['ctg']))) {
			...
			$out = '<< /Length '.strlen($stream).'';
			if (substr($fontfile, -2) == '.z') { 																$out .= ' /Filter /FlateDecode';
			}
			$out .= ' >>';
			$out .= ' stream'."\n".$stream."\n".'endstream';
			$out .= "\n".'endobj';
	...
	protected function _putxobjects() {
		foreach ($this->xobjects as $key => $data) {
			if (isset($data['outdata'])) {
				...
				$out .= ' >>'; 				if (isset($data['group']) AND ($data['group'] !== false)) {
	...
	protected function _enddoc() {
		...
		$this->_putresources();
			...
			foreach ($this->empty_signature_appearance as $key => $esa) {
				...
				$out .= ' /Subtype /Widget';
				$out .= ' /Rect ['.$esa['rect'].']';
				$out .= ' /P '.$this->page_obj_id[($esa['page'])].' 0 R'; 				$out .= ' /F 4';
				$out .= ' /FT /Sig';
			...
			$out .= ' /P '.$this->page_obj_id[($this->signature_appearance['page'])].' 0 R'; 			$out .= ' /F 4';
			$out .= ' /FT /Sig';
	...
	protected function _objectkey($n) {
		...
		if ($this->encryptdata['mode'] == 2) { 						$objkey .= "\x73\x41\x6C\x54"; 		}
	...
	protected function _encrypt_data($n, $s) {
		...
		switch ($this->encryptdata['mode']) {
			case 0:   			case 1: { 				$s = TCPDF_STATIC::_RC4($this->_objectkey($n), $s, $this->last_enc_key, $this->last_enc_key_c);
				break;
			}
			case 2: { 				$s = TCPDF_STATIC::_AES($this->_objectkey($n), $s);
				break;
			}
			case 3: { 				$s = TCPDF_STATIC::_AES($this->encryptdata['key'], $s);
				break;
			}
		}
		return $s;
	...
	protected function _putencryption() {
		...
		if ($this->encryptdata['V'] >= 4) {
			...
			} elseif ($this->encryptdata['V'] == 4) { 				$out .= ' 4';
			} elseif ($this->encryptdata['V'] < 2) { 				$out .= ' 2';
			} else { 				$out .= ' 3';
			}
	...
	protected function _Uvalue() {
		if ($this->encryptdata['mode'] == 0) { 			return TCPDF_STATIC::_RC4($this->encryptdata['key'], TCPDF_STATIC::$enc_padding, $this->last_enc_key, $this->last_enc_key_c);
		} elseif ($this->encryptdata['mode'] < 3) { 			$tmp = TCPDF_STATIC::_md5_16(TCPDF_STATIC::$enc_padding.$this->encryptdata['fileid']);
		...
		} elseif ($this->encryptdata['mode'] == 3) { 			$seed = TCPDF_STATIC::_md5_16(TCPDF_STATIC::getRandomSeed());
	...
	protected function _Ovalue() {
		if ($this->encryptdata['mode'] < 3) { 			$tmp = TCPDF_STATIC::_md5_16($this->encryptdata['owner_password']);
	...
	protected function _fixAES256Password($password) {
		$psw = ''; 		$psw_array = TCPDF_FONTS::utf8Bidi(TCPDF_FONTS::UTF8StringToArray($password, $this->isunicode, $this->CurrentFont), $password, $this->rtl, $this->isunicode, $this->CurrentFont);
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { 			if ($this->encryptdata['mode'] == 3) { 								$this->encryptdata['key'] = substr(hash('sha256', TCPDF_STATIC::getRandomSeed(), true), 0, $keybytelen);
				...
				$this->encryptdata['owner_password'] = $this->_fixAES256Password($this->encryptdata['owner_password']);
								...
								$perms = TCPDF_STATIC::getEncPermissionsString($this->encryptdata['protection']); 				$perms .= chr(255).chr(255).chr(255).chr(255); 				if (isset($this->encryptdata['CF']['EncryptMetadata']) AND (!$this->encryptdata['CF']['EncryptMetadata'])) { 					$perms .= 'F';
				} else {
					$perms .= 'T';
				}
				$perms .= 'adb'; 				$perms .= 'nick'; 				$this->encryptdata['perms'] = TCPDF_STATIC::_AESnopad($this->encryptdata['key'], $perms);
			} else { 								$this->encryptdata['user_password'] = substr($this->encryptdata['user_password'].TCPDF_STATIC::$enc_padding, 0, 32);
		...
		} else { 						$seed = sha1(TCPDF_STATIC::getRandomSeed(), true);
			$recipient_bytes = '';
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				...
				unset($tmparr);
						...
						if ($this->encryptdata['mode'] == 3) { 				$this->encryptdata['key'] = substr(hash('sha256', $seed.$recipient_bytes, true), 0, $keybytelen);
			} else { 				$this->encryptdata['key'] = substr(sha1($seed.$recipient_bytes, true), 0, $keybytelen);
			}
		}
	}
	...
	public function SetProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		if ($mode > 1) { 			if (!extension_loaded('openssl') && !extension_loaded('mcrypt')) {
		...
		switch ($mode) {
			case 0: { 				$this->encryptdata['V'] = 1;
				$this->encryptdata['Length'] = 40;
				$this->encryptdata['CF']['CFM'] = 'V2';
				break;
			}
			case 1: { 				$this->encryptdata['V'] = 2;
	...
	public function setSignature($signing_cert='', $private_key='', $private_key_password='', $extracerts='', $cert_type=2, $info=array(), $approval='') {
										$this->sign = true;
		++$this->n;
		$this->sig_obj_id = $this->n; 		++$this->n; 		$this->signature_data = array();
	...
	public function colorRegistrationBar($x, $y, $w, $h, $transition=true, $vertical=false, $colors='A,R,G,B,C,M,Y,K') {
		...
		$bars = explode(',', $colors);
		$numbars = count($bars); 		if ($numbars <= 0) {
			return;
		}
				if ($vertical) {
			$coords = array(0, 0, 0, 1);
			$wb = $w / $numbars; 			$hb = $h; 			$xd = $wb; 			$yd = 0; 		} else {
			$coords = array(1, 0, 0, 0);
			$wb = $w; 			$hb = $h / $numbars; 			$xd = 0; 			$yd = $hb; 		}
		$xb = $x;
		$yb = $y;
		foreach ($bars as $col) {
			switch ($col) {
								case 'A': { 					$col_a = array(255);
					$col_b = array(0);
					break;
				}
				case 'W': { 					$col_a = array(0);
					$col_b = array(255);
					break;
				}
				case 'R': { 					$col_a = array(255,255,255);
					$col_b = array(255,0,0);
					break;
				}
				case 'G': { 					$col_a = array(255,255,255);
					$col_b = array(0,255,0);
					break;
				}
				case 'B': { 					$col_a = array(255,255,255);
					$col_b = array(0,0,255);
					break;
				}
				case 'C': { 					$col_a = array(0,0,0,0);
					$col_b = array(100,0,0,0);
					break;
				}
				case 'M': { 					$col_a = array(0,0,0,0);
					$col_b = array(0,100,0,0);
					break;
				}
				case 'Y': { 					$col_a = array(0,0,0,0);
					$col_b = array(0,0,100,0);
					break;
				}
				case 'K': { 					$col_a = array(0,0,0,0);
					$col_b = array(0,0,0,100);
					break;
				}
				case 'RGB': { 					$col_a = array(255,255,255);
					$col_b = array(0,0,0);
					break;
				}
				case 'CMYK': { 					$col_a = array(0,0,0,0);
					$col_b = array(100,100,100,100);
					break;
				}
				case 'ALL': { 					$col_a = array(0,0,0,0,'None');
					$col_b = array(100,100,100,100,'All');
					break;
				}
				case 'NONE': { 					$col_a = array(0,0,0,0,'None');
					$col_b = array(0,0,0,0,'None');
					break;
				}
				default: { 					$col_a = array(0,0,0,0,'None');
	...
	public function cropMark($x, $y, $w, $h, $type='T,R,B,L', $color=array(100,100,100,100,'All')) {
		...
		$dw = ($w / 4); 		$dh = ($h / 4); 		foreach ($crops as $crop) {
			switch ($crop) {
				case 'T':
				case 'TOP': {
					$x1 = $x;
	...
	public function CoonsPatchMesh($x, $y, $w, $h, $col1=array(), $col2=array(), $col3=array(), $col4=array(), $coords=array(0.00,0.0,0.33,0.00,0.67,0.00,1.00,0.00,1.00,0.33,1.00,0.67,1.00,1.00,0.67,1.00,0.33,1.00,0.00,1.00,0.00,0.67,0.00,0.33), $coords_min=0, $coords_max=1, $antialias=false) {
		...
		$this->gradients[$n]['type'] = 6; 		$this->gradients[$n]['coords'] = array();
		...
		$bpcd = 65535; 				$this->gradients[$n]['stream'] = '';
		$count_patch = count($patch_array);
		for ($i=0; $i < $count_patch; ++$i) {
			$this->gradients[$n]['stream'] .= chr($patch_array[$i]['f']); 			$count_points = count($patch_array[$i]['points']);
	...
	public function Gradient($type, $coords, $stops, $background=array(), $antialias=false) {
		...
		foreach ($stops as $key => $stop) {
			...
			switch($numcolspace) {
				case 5:   				case 4: { 					$this->gradients[$n]['colors'][$key]['color'] = sprintf('%F %F %F %F', $color[0]/100, $color[1]/100, $color[2]/100, $color[3]/100);
					break;
				}
				case 3: { 					$this->gradients[$n]['colors'][$key]['color'] = sprintf('%F %F %F', $color[0]/255, $color[1]/255, $color[2]/255);
					break;
				}
				case 1: { 					$this->gradients[$n]['colors'][$key]['color'] = sprintf('%F', $color[0]/255);
					break;
				}
			}
		}
	...
	function _putshaders() {
		...
		$idt = count($this->gradients); 		foreach ($this->gradients as $id => $grad) {
	...
	public function ImageEps($file, $x='', $y='', $w=0, $h=0, $link='', $useBoundingBox=true, $align='', $palign='', $border=0, $fitonpage=false, $fixoutvals=false) {
		...
		if ($y === '') {
			$y = $this->y;
				...
				list($x, $y) = $this->checkPageRegions($h, $x, $y);
		$k = $this->k;
		if ($file[0] === '@') { 			$data = substr($file, 1);
		} else { 			$data = TCPDF_STATIC::fileGetContents($file);
		}
		if ($data === FALSE) {
			$this->Error('EPS file not found: '.$file);
		}
		$regs = array();
				preg_match("/%%Creator:([^\r\n]+)/", $data, $regs); 		if (count($regs) > 1) {
			$version_str = trim($regs[1]); 			if (strpos($version_str, 'Adobe Illustrator') !== false) {
		...
		for ($i=0; $i < $cnt; ++$i) {
			...
			$cmd = trim(array_pop($chunks));
				...
				$g = array_pop($chunks);
				$r = array_pop($chunks);
				$this->_out(''.$r.' '.$g.' '.$b.' '.($cmd=='Xa'?'rg':'RG')); 				continue;
			}
			$skip = false;
			if ($fixoutvals) {
								switch ($cmd) {
	...
	protected function getCSSBorderStyle($cssborder) {
		...
		$border = array(); 		switch (count($bprop)) {
			case 3: {
				$width = $bprop[0];
				$style = $bprop[1];
				$color = $bprop[2];
	...
	public function getCSSPadding($csspadding, $width=0) {
		...
		$cell_padding = array(); 		switch (count($padding)) {
			case 4: {
	...
	public function getCSSMargin($cssmargin, $width=0) {
		...
		$cell_margin = array(); 		switch (count($margin)) {
			case 4: {
	...
	public function getCSSBorderMargin($cssbspace, $width=0) {
		...
		$border_spacing = array(); 		switch (count($space)) {
			case 2: {
	...
	protected function getHtmlDomArray($html) {
		...
		$html = preg_replace('/<style([^\>]*)>([^\<]*)<\/style>/isU', '', $html);
				...
				$html = preg_replace('/<pre/', '<xre', $html); 		$html = preg_replace('/<(table|tr|td|th|tcpdf|blockquote|dd|div|dl|dt|form|h1|h2|h3|h4|h5|h6|br|hr|li|ol|ul|p)([^\>]*)>[\n\r\t]+/', '<\\1\\2>', $html);
		$html = preg_replace('@(\r\n|\r)@', "\n", $html);
		...
		$html = preg_replace('/<xre/', '<pre', $html); 		$html = preg_replace('/<textarea([^\>]*)>([^\<]*)<\/textarea>/xi', '<textarea\\1 value="\\2" />', $html);
		...
		$html = preg_replace('/<([^\>\/]*)>[\s]/', '<\\1>&nbsp;', $html); 		$html = preg_replace('/[\s]<\/([^\>]*)>/', '&nbsp;</\\1>', $html); 		$html = preg_replace('/<su([bp])/', '<zws/><su\\1', $html); 		$html = preg_replace('/<\/su([bp])>/', '</su\\1><zws/>', $html); 		$html = preg_replace('/'.$this->re_space['p'].'+/'.$this->re_space['m'], chr(32), $html); 				$html = $this->stringTrim($html);
		...
		$dom[$key]['fgcolor'] = $this->fgcolor; 		$dom[$key]['strokecolor'] = $this->strokecolor;
		$dom[$key]['align'] = '';
		$dom[$key]['listtype'] = '';
		...
		$thead = false; 		++$key;
		$level = array();
		array_push($level, 0); 		while ($elkey < $maxel) {
			$dom[$key] = array();
			$element = $a[$elkey];
			$dom[$key]['elkey'] = $elkey;
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					$dom[$key]['attribute'] = array(); 					while (list($id, $name) = each($attr_array[1])) {
					...
					if (!empty($css)) {
						...
						$dom[$key]['style'] = array(); 						while (list($id, $name) = each($style_array[1])) {
					...
					if (($dom[$key]['value'] == 'table')) {
						$dom[$key]['rows'] = 0; 						$dom[$key]['trids'] = array(); 						$dom[$key]['thead'] = ''; 					}
					if (($dom[$key]['value'] == 'tr')) {
						$dom[$key]['cols'] = 0;
						if ($thead) {
							$dom[$key]['thead'] = true;
	...
	public function writeHTML($html, $ln=true, $fill=false, $reseth=false, $cell=false, $align='') {
		...
		$undo = false;
		$fontaligned = false;
		$reverse_dir = false; 		$this->premode = false;
		if ($this->inxobj) {
		...
		while ($key < $maxel) {
			...
			if ($dom[$key]['tag'] OR ($key == 0)) {
				...
				$fontaligned = false;
					...
					if ((isset($plalign) AND ((($plalign == 'C') OR ($plalign == 'J') OR (($plalign == 'R') AND (!$this->rtl)) OR (($plalign == 'L') AND ($this->rtl)))))) {
						...
						$no = 0; 						if ($this->isRTLTextDir()) { 														$pos1 = TCPDF_STATIC::revstrpos($pmid, '[(');
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
						...
						} else { 														$pos1 = TCPDF_STATIC::revstrpos($pmid, ')]');
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
								...
								while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
									...
									$currentxpos = 0;
										...
										case 're': {
											...
											$x_diff = 0;
											$w_diff = 0;
											if ($this->isRTLTextDir()) { 												if ($currentxpos < $textpos) {
			...
			if ($dom[$key]['tag']) {
				...
				} else { 					$prev_numpages = $this->numpages;
				...
				if ((!$this->premode) AND $this->isRTLTextDir()) {
										$lsp = ''; 					$rsp = ''; 					if (preg_match('/^('.$this->re_space['p'].'+)/'.$this->re_space['m'], $dom[$key]['value'], $matches)) {
						$lsp = $matches[1];
					}
						...
						if (($wadj > 0) AND (($strlinelen + $wadj) >= $cwa)) {
							...
							if ($numblks > 1) {
												...
												if (($wadj > 0) AND (($this->rtl AND ($this->tmprtl === 'L')) OR (!$this->rtl AND ($this->tmprtl === 'R')))) {
														$reverse_dir = true;
							$this->rtl = !$this->rtl;
							$revshift = ($strlinelen + $wadj + 0.000001); 							if ($this->rtl) {
								$this->x += $revshift;
							} else {
								$this->x -= $revshift;
							}
	...
	protected function openHTMLTagHandler($dom, $key, $cell) {
		...
		if ($tag['block']) {
			$hbz = 0; 			$hb = 0; 						if (isset($this->tagvspaces[$tag['value']][0]['h']) AND ($this->tagvspaces[$tag['value']][0]['h'] >= 0)) {
	...
	protected function closeHTMLTagHandler($dom, $key, $cell, $maxbottomliney=0) {
		...
		if ($tag['block']) {
			$hbz = 0; 			$hb = 0; 						if (isset($this->tagvspaces[$tag['value']][1]['h']) AND ($this->tagvspaces[$tag['value']][1]['h'] >= 0)) {
				$pre_h = $this->tagvspaces[$tag['value']][1]['h'];
			} elseif (isset($parent['fontsize'])) {
				$pre_h = $this->getCellHeight($parent['fontsize'] / $this->k);
			} else {
			...
			case 'table': {
				...
				foreach ($table_el['trids'] as $j => $trkey) {
					...
					if (!isset($xmax)) {
						...
						$border_middle = TCPDF_STATIC::getBorderMode($border, $position='middle', $this->opencell);
							...
							if ($startpage == $endpage) { 								$deltacol = 0;
								$deltath = 0;
								for ($column = $startcolumn; $column <= $endcolumn; ++$column) { 									$this->selectColumn($column);
									if ($startcolumn == $endcolumn) { 										$cborder = $border;
									...
									$ccode .= $this->getCellCode($w, $h, '', $cborder, 1, '', $fill, '', 0, true)."\n";
								} 							} elseif ($page == $startpage) { 								$deltacol = 0;
								$deltath = 0;
								for ($column = $startcolumn; $column < $this->num_columns; ++$column) { 									$this->selectColumn($column);
									if ($column == $startcolumn) { 										$cborder = $border_start;
										$this->y = $starty;
										$this->x = $x;
										$h = $this->h - $this->y - $this->bMargin;
									...
									$ccode .= $this->getCellCode($w, $h, '', $cborder, 1, '', $fill, '', 0, true)."\n";
								} 							} elseif ($page == $endpage) { 								$deltacol = 0;
								$deltath = 0;
								for ($column = 0; $column <= $endcolumn; ++$column) { 									$this->selectColumn($column);
									if ($column == $endcolumn) { 										$cborder = $border_end;
										if (isset($this->columns[$column]['th']['\''.$page.'\''])) {
											$this->y = $this->columns[$column]['th']['\''.$page.'\''];
										}
										$this->x += $deltacol;
								...
								$deltath = 0;
								for ($column = 0; $column < $this->num_columns; ++$column) { 									$this->selectColumn($column);
									$cborder = $border_middle;
									if (isset($this->columns[$column]['th']['\''.$page.'\''])) {
										$this->y = $this->columns[$column]['th']['\''.$page.'\''];
									}
				...
				if (!$in_table_head) { 					$this->cell_padding = $table_el['old_cell_padding'];
										$this->resetLastH();
	...
	protected function putHtmlListBullet($listdepth, $listtype='', $size=10) {
		...
		switch ($listtype) {
			...
			case 'circle': {
				...
				$this->_out($prev_line_style); 				break;
			}
			case 'square': {
				$l = $size / 3;
				$lspace += $l;
	...
	public function addTOC($page='', $numbersfont='', $filler='.', $toc_name='TOC', $style='', $color=array(0,0,0)) {
		...
		$maxpage = 0; 		foreach ($this->outlines as $key => $outline) {
	...
	protected function hyphenateWord($word, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		$hyphenword = array(); 		$numchars = count($word);
		if ($numchars <= $charmin) {
			return $word;
		}
	...
	public function hyphenateText($text, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		$word = array(); 		$txtarr = array(); 		$intag = false; 		$skip = false; 		if (!is_array($patterns)) {
			...
			if ((!$intag) AND (!$skip) AND TCPDF_FONT_DATA::$uni_type[$char] == 'L') {
				...
				if (chr($char) == '<') {
										...
										$intag = false;
										$expected = array(115, 116, 121, 108, 101); 					$current = array_slice($txtarr, -6, 5); 					$compare = array_diff($expected, $current);
					if (empty($compare)) {
												$expected = array(47); 						$current = array_slice($txtarr, -7, 1);
		...
		if ($this->num_columns > 1) {
			...
			if ($regdata['page'] == $this->page) {
								if (($y > ($regdata['yt'] - $h)) AND ($y <= $regdata['yb'])) {
										$minv = ($regdata['xb'] - $regdata['xt']) / ($regdata['yb'] - $regdata['yt']); 					$yt = max($y, $regdata['yt']);
					$yb = min(($yt + $h), $regdata['yb']);
					$xt = (($yt - $regdata['yt']) * $minv) + $regdata['xt'];
					$xb = (($yb - $regdata['yt']) * $minv) + $regdata['xt'];
					if ($regdata['side'] == 'L') { 						$new_margin = max($xt, $xb);
					...
					} elseif ($regdata['side'] == 'R') { 						$new_margin = min($xt, $xb);
	...
	public function ImageSVG($file, $x='', $y='', $w=0, $h=0, $link='', $align='', $palign='', $border=0, $fitonpage=false) {
		...
		if ($file[0] === '@') { 			$this->svgdir = '';
			$svgdata = substr($file, 1);
		} else { 			$this->svgdir = dirname($file);
	...
	protected function setSVGStyles($svgstyle, $prevsvgstyle, $x=0, $y=0, $w=1, $h=1, $clip_function='', $clip_params=array()) {
		...
		$objstyle = '';
		$minlen = (0.01 / $this->k); 		if (!isset($svgstyle['opacity'])) {
			return $objstyle;
		}
				$regs = array();
	...
	protected function SVGPath($d, $style='') {
		...
		$ymax = 0;
		$relcoord = false;
		$minlen = (0.01 / $this->k); 		$firstcmd = true; 				foreach ($paths as $key => $val) {
						$cmd = trim($val[1]);
			...
			switch (strtoupper($cmd)) {
				case 'M': { 					foreach ($params as $ck => $cp) {
						if (($ck % 2) == 0) {
							$x = $cp + $xoffset;
						} else {
							$y = $cp + $yoffset;
				...
				case 'L': { 					foreach ($params as $ck => $cp) {
						if (($ck % 2) == 0) {
							$x = $cp + $xoffset;
						} else {
							$y = $cp + $yoffset;
				...
				case 'H': { 					foreach ($params as $ck => $cp) {
						$x = $cp + $xoffset;
						if ((abs($x0 - $x) >= $minlen) OR (abs($y0 - $y) >= $minlen)) {
							$this->_outLine($x, $y);
							$x0 = $x;
				...
				case 'V': { 					foreach ($params as $ck => $cp) {
						$y = $cp + $yoffset;
						if ((abs($x0 - $x) >= $minlen) OR (abs($y0 - $y) >= $minlen)) {
							$this->_outLine($x, $y);
							$x0 = $x;
				...
				case 'C': { 					foreach ($params as $ck => $cp) {
						$params[$ck] = $cp;
						if ((($ck + 1) % 6) == 0) {
				...
				case 'S': { 					foreach ($params as $ck => $cp) {
						$params[$ck] = $cp;
						if ((($ck + 1) % 4) == 0) {
				...
				case 'Q': { 					foreach ($params as $ck => $cp) {
						$params[$ck] = $cp;
						if ((($ck + 1) % 4) == 0) {
				...
				case 'T': { 					foreach ($params as $ck => $cp) {
						$params[$ck] = $cp;
						if (($ck % 2) != 0) {
				...
				case 'A': { 					foreach ($params as $ck => $cp) {
						$params[$ck] = $cp;
						if ((($ck + 1) % 7) == 0) {
							$x0 = $x;
							$y0 = $y;
							...
							$ang = -$rawparams[($ck - 4)];
							$angle = deg2rad($ang);
							$fa = $rawparams[($ck - 3)]; 							$fs = $rawparams[($ck - 2)]; 							$x = $params[($ck - 1)] + $xoffset;
							$y = $params[$ck] + $yoffset;
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		if ($parser == 'clip-path') {
				...
				$prev_svgstyle = $this->svgstyles[max(0,(count($this->svgstyles) - 1))]; 		$svgstyle = $this->svgstyles[0]; 		if ($clipping AND !isset($attribs['fill']) AND (!isset($attribs['style']) OR (!preg_match('/[;\"\s]{1}fill[\s]*:[\s]*([^;\"]*)/si', $attribs['style'], $attrval)))) {
						$attribs['fill'] = 'none';
		}
		...
		if (($svgstyle['visibility'] == 'hidden') OR ($svgstyle['visibility'] == 'collapse') OR ($svgstyle['display'] == 'none')) {
			...
			case 'g': {
				...
				$w = 1;				$h = 1;				$tm = TCPDF_STATIC::getTransformationMatrixProduct($tm, array($w, 0, 0, $h, $x, $y));
				$this->SVGTransform($tm);
				$this->setSVGStyles($svgstyle, $prev_svgstyle);
				break;
			}
