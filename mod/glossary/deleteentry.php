...
$id       = required_param('id', PARAM_INT);          $confirm  = optional_param('confirm', 0, PARAM_INT);  $entry    = optional_param('entry', 0, PARAM_INT);    $prevmode = required_param('prevmode', PARAM_ALPHA);
...
if (($entry->userid != $USER->id) and !$manageentries) {     print_error('nopermissiontodelentry');
}
...
} else {            $PAGE->navbar->add(get_string('delete'));
