<?php
    ...
    $id    = required_param('id', PARAM_INT);        $type  = required_param('type', PARAM_FILE);      $group = optional_param('group', 0, PARAM_INT);      $sid   = optional_param('sid', false, PARAM_INT);      $qid   = optional_param('qid', 0, PARAM_INT);  
    ...
    $groupmode = groups_get_activity_groupmode($cm);       $context = context_module::instance($cm->id);
    ...
    switch ($type) {
     ...
     case "overall.png":
       ...
       for ($i=0; $i<$numquestions; $i++) {
           ...
           $count1[$i] = 0;
           $count2[$i] = 0;
           $subquestions = $question[$i]->multi;              $aaa = $DB->get_records_select("survey_answers", "((survey = ?) AND (question in ($subquestions)))", array($cm->instance));
           ...
           $buckets1[$i] = $buckets1[$i] - 1;                    $buckets2[$i] = $buckets2[$i] - 1;
