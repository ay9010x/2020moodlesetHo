...
$id     = optional_param('id', 0, PARAM_INT);           if ($id) {
    $cm = get_coursemodule_from_id('data', $id, null, null, MUST_EXIST);
    $course = $DB->get_record('course', array('id'=>$cm->course), '*', MUST_EXIST);
    $data = $DB->get_record('data', array('id'=>$cm->instance), '*', MUST_EXIST);
} else {
    $d = required_param('d', PARAM_INT);         $data = $DB->get_record('data', array('id'=>$d), '*', MUST_EXIST);
...
if (optional_param('sesskey', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if ($formdata = $form_importexisting->get_data()) {
        ...
        $fullname = optional_param('fullname', '', PARAM_PATH);                         $parts = explode('/', $fullname, 2);
