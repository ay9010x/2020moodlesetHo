...
if ($groupmode = groups_get_activity_groupmode($cm)) {       $currentgroup = groups_get_activity_group($cm);
} else {
    $currentgroup = 0;
}
        ...
        foreach ($questions as $question) {
            ...
            if ($question->type == 0 or $question->type == 1) {
                if ($answer = survey_get_user_answer($survey->id, $question->id, $USER->id)) {
                    ...
                    $table->data[] = array(s($answer->answer1));                    echo html_writer::table($table);
                    echo $OUTPUT->spacer(array('height' => 30, 'width' => 1), true);
                }
            }
        }
...
global $qnum;  global $checklist; $qnum = 0;
$checklist = array();
foreach ($questions as $question) {
