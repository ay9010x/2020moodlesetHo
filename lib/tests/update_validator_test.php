...
class core_update_validator_testcase extends advanced_testcase {
    ...
    public function test_validate_version_php() {
        ...
        $this->assertEquals('MATURITY_ALPHA', $versionphpinfo['maturity']);         $this->assertEquals(MATURITY_ALPHA, constant($versionphpinfo['maturity']));         $this->assertTrue($this->has_message($validator->get_messages(), $validator::WARNING, 'maturity', 'MATURITY_ALPHA'));
    }
    ...
    public function test_parse_version_php() {
        ...
        $this->assertEquals('block_foobar', $info['plugin->component']);          $this->assertEquals('2013010100', $info['plugin->version']);              $this->assertEquals('2012122401', $info['plugin->requires']);             $this->assertEquals('MATURITY_STABLE', $info['module->maturity']);        $this->assertEquals('MATURITY_ALPHA', $info['plugin->maturity']);         $this->assertEquals('v2.3', $info['module->release']);                    $this->assertEquals('v2.4', $info['plugin->release']);                }
