...
class lesson_page_type_branchtable extends lesson_page {
    ...
    public function check_answer() {
        ...
        if($newpageid == LESSON_UNSEENBRANCHPAGE) {
            if (has_capability('mod/lesson:manage', $context)) {
                 ...
                 $newpageid = lesson_unseen_question_jump($this->lesson, $USER->id, $this->properties->id);              }
        }
                if ($newpageid == 0) {
    ...
    public function report_answers($answerpage, $answerdata, $useranswer, $pagestats, &$i, &$n) {
        ...
        $formattextdefoptions->para = false;          $formattextdefoptions->context = $answerpage->context;
    ...
    public function is_unseen($param) {
        ...
        if (is_array($param)) {
            ...
            foreach ($branchpages as $branchpage) {
                if (array_key_exists($branchpage->id, $seenpages)) {                      return false;
                }
            }
            return true;
        } else {
