...
class core_test_generator_testcase extends advanced_testcase {
    ...
    public function test_create_module() {
        ...
        $generator->create_grouping_group(array('groupid' => $group->id, 'groupingid' => $grouping->id));
            ...
            'visible' => 0,             'section' => 3,                         'cmidnumber' => 'IDNUM',                         'groupmode' => SEPARATEGROUPS,                         'groupingid' => $grouping->id,
        );

                $featurecompletionmanual = array(
            'completion' => COMPLETION_TRACKING_MANUAL,             'completionexpected' => time() + 7 * DAYSECS,
        );
            ...
            'completion' => COMPLETION_TRACKING_AUTOMATIC,             'completionview' => 1,             'completionusegrade' => 1,         );
                ...
                $featurerate = array(
            'assessed' => RATING_AGGREGATE_AVERAGE,             'scale' => 100,             'ratingtime' => 1,             'assesstimestart' => time() - DAYSECS,             'assesstimefinish' => time() + DAYSECS,         );

                $featuregrade = array(
            'grade' => 10,
            'gradecat' => $grade_category->id,         );
        ...
        $this->assertEquals($optionsgeneral['section'], $cm1->sectionnum);         $this->assertEquals($optionsgeneral['cmidnumber'], $cm1->idnumber);         $this->assertEquals($optionsgeneral['groupmode'], $cm1->groupmode);
        ...
        $this->assertEquals(0, $cm3->completiongradeitemnumber);         $gradingitem = grade_item::fetch(array('courseid'=>$course->id, 'itemtype'=>'mod', 'itemmodule' => 'assign', 'iteminstance' => $m3->id));
