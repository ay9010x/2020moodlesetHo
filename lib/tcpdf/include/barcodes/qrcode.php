...
class QRcode {
	...
	protected $formatInfo = array(
		array(0x77c4, 0x72f3, 0x7daa, 0x789d, 0x662f, 0x6318, 0x6c41, 0x6976), 		array(0x5412, 0x5125, 0x5e7c, 0x5b4b, 0x45f9, 0x40ce, 0x4f97, 0x4aa0), 		array(0x355f, 0x3068, 0x3f31, 0x3a06, 0x24b4, 0x2183, 0x2eda, 0x2bed), 		array(0x1689, 0x13be, 0x1ce7, 0x19d0, 0x0762, 0x0255, 0x0d0c, 0x083b)  	);
	...
	protected function generateMaskNo($maskNo, $width, $frame) {
	 ...
	 protected function eatNum() {
		...
		if ($mode == QR_MODE_8B) {
			...
			+ $this->estimateBitsMode8(1)         			- $this->estimateBitsMode8($run + 1); 			if ($dif > 0) {
				return $this->eat8();
			}
		}
		if ($mode == QR_MODE_AN) {
			$dif = $this->estimateBitsModeNum($run) + 4 + $ln
			+ $this->estimateBitsModeAn(1)        			- $this->estimateBitsModeAn($run + 1);			if ($dif > 0) {
				return $this->eatAn();
			}
		}
	 ...
	 protected function eatAn() {
		...
		while($this->isalnumat($this->dataStr, $p)) {
			if ($this->isdigitat($this->dataStr, $p)) {
				...
				$dif = $this->estimateBitsModeAn($p) 				+ $this->estimateBitsModeNum($q - $p) + 4 + $ln
				- $this->estimateBitsModeAn($q); 				if ($dif < 0) {
					break;
				} else {
					$p = $q;
				}
	 ...
	 protected function eat8() {
		...
		while($p < $dataStrLen) {
			...
			if ($mode == QR_MODE_NM) {
				...
				$dif = $this->estimateBitsMode8($p) 				+ $this->estimateBitsModeNum($q - $p) + 4 + $ln
				- $this->estimateBitsMode8($q); 				if ($dif < 0) {
					break;
				} else {
					$p = $q;
				}
				...
				$dif = $this->estimateBitsMode8($p)  				+ $this->estimateBitsModeAn($q - $p) + 4 + $la
				- $this->estimateBitsMode8($q); 				if ($dif < 0) {
					break;
				} else {
					$p = $q;
	...
	protected function init_rs_char($symsize, $gfpoly, $fcr, $prim, $nroots, $pad) {
	 ...
	 protected function encode_rs_char($rs, $data, $parity) {
		$MM       =& $rs['mm']; 		$NN       =& $rs['nn']; 		$ALPHA_TO =& $rs['alpha_to']; 		$INDEX_OF =& $rs['index_of']; 		$GENPOLY  =& $rs['genpoly']; 		$NROOTS   =& $rs['nroots']; 		$FCR      =& $rs['fcr']; 		$PRIM     =& $rs['prim']; 		$IPRIM    =& $rs['iprim']; 		$PAD      =& $rs['pad']; 		$A0       =& $NN;
