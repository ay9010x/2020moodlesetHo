...
require_once(dirname(dirname(__FILE__)) . '/lib.php');                  require_once(dirname(dirname(dirname(__FILE__))) . '/locallib.php');    
...
class workshop_manual_allocator implements workshop_allocator {
    ...
    public function init() {
        ...
        case 'new':
            ...
            $m          = array();              $submission = $this->workshop->get_submission_by_author($authorid);
            if (!$submission) {
            ...
            $m = implode('-', $m);              redirect($PAGE->url->out(false, array('m' => $m)));
            break;
        case 'del':
            if (!confirm_sesskey()) {
                throw new moodle_exception('confirmsesskeybad');
    ...
    public function ui() {
        ...
        $hlauthorid     = -1;                   $hlreviewerid   = -1;           
        $message        = new workshop_message();

        $m  = optional_param('m', '', PARAM_ALPHANUMEXT);           if ($m) {
            $m = explode('-', $m);
            switch ($m[0]) {
            case self::MSG_ADDED:
                $hlauthorid     = $m[1];
