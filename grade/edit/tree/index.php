...
if ($data = data_submitted() and confirm_sesskey()) {
    ...
    $DB->set_field('grade_categories_setuppage', 'setuppage', 1, array('courseid' => $courseid));
...
print_grade_page_head($courseid, 'settings', 'setup', get_string('gradebooksetup', 'grades'), '', '', false);   // disable the selector
...
// Transfer back to the simple Gradebook setup page.  // by YCJ
if (!$moving && count($grade_edit_tree->categories) == 1) {
//	  $gtree_element = $gtree->top_element;
//    echo $OUTPUT->single_button(new moodle_url('../moodleset/transfer.php', array('courseid'=>$course->id, 'catid'=>$gtree_element['object']->id)), get_string('gradebook_simplesetup', 'grades'), 'get');
    echo $OUTPUT->single_button(new moodle_url('../moodleset/transfer.php', array('id'=>$course->id)), get_string('gradebook_simplesetup', 'grades'), 'get');
}
