...
class PDF417 {
	...
	public function __construct($code, $ecl=-1, $aspectratio=2, $macro=array()) {
		...
		$codewords = array(); 		foreach($sequence as $seq) {
	...
	protected function getErrorCorrectionLevel($ecl, $numcw) {
				$maxecl = 8; 		$maxerrsize = (928 - $numcw); 		while ($maxecl > 0) {
			$errsize = (2 << $ecl);
	...
	protected function getInputSequences($code) {
		$sequence_array = array(); 		$numseq = array();
	...
	protected function getCompaction($mode, $code, $addmode=true) {
		$cw = array(); 		switch($mode) {
			case 900: { 				$submode = 0; 				$txtarr = array(); 				$codelen = strlen($code);
				for ($i = 0; $i < $codelen; ++$i) {
					$chval = ord($code{$i});
			...
			case 901:
			case 924: { 				while (($codelen = strlen($code)) > 0) {
					if ($codelen > 6) {
						$rest = substr($code, 6);
			...
			case 902: { 				while (($codelen = strlen($code)) > 0) {
					if ($codelen > 44) {
						$rest = substr($code, 44);
