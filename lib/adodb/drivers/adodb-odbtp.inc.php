...
class ADODB_odbtp extends ADOConnection{
	...
	var $replaceQuote = "''"; 	var $odbc_driver = 0;
	...
	function SelectDB($dbName)
	{
		...
		$this->databaseName = $dbName; 		return true;
	}
	...
	global $ADODB_FETCH_MODE;
		...
		while (!$rs->EOF) {
				...
				if (isset($rs->fields[12])) 	 				if (!is_null($rs->fields[12])) {
	...
	function CommitTrans($ok=true)
	{
		...
		if( ($ret = @odbtp_commit($this->_connectionID)) )
			$ret = @odbtp_set_attr(ODB_ATTR_TRANSACTIONS, ODB_TXN_NONE, $this->_connectionID);		return $ret;
	}
	...
	{
		if (! $this->_bindInputArray) return $sql; 
	...
	{
		if (!$this->_canPrepareSP) return $sql; 
        $this->_errorMsg = false;
		$this->_errorCode = false;
...
class ADORecordSet_odbtp extends ADORecordSet {
	...
	{
		$off=$fieldOffset; 		$o= new ADOFieldObject();
