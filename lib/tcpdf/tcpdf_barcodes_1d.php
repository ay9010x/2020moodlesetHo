...
class TCPDFBarcode {
	...
	public function getBarcodeSVG($w=2, $h=30, $color='black') {
		...
		header('Cache-Control: public, must-revalidate, max-age=0'); 		header('Pragma: public');
	...
	public function setBarcode($code, $type) {
		switch (strtoupper($type)) {
			case 'C39': { 				$arrcode = $this->barcode_code39($code, false, false);
				break;
			}
			case 'C39+': { 				$arrcode = $this->barcode_code39($code, false, true);
				break;
			}
			case 'C39E': { 				$arrcode = $this->barcode_code39($code, true, false);
				break;
			}
			case 'C39E+': { 				$arrcode = $this->barcode_code39($code, true, true);
				break;
			}
			case 'C93': { 				$arrcode = $this->barcode_code93($code);
				break;
			}
			case 'S25': { 				$arrcode = $this->barcode_s25($code, false);
				break;
			}
			case 'S25+': { 				$arrcode = $this->barcode_s25($code, true);
				break;
			}
			case 'I25': { 				$arrcode = $this->barcode_i25($code, false);
				break;
			}
			case 'I25+': { 				$arrcode = $this->barcode_i25($code, true);
				break;
			}
			case 'C128': { 				$arrcode = $this->barcode_c128($code, '');
				break;
			}
			case 'C128A': { 				$arrcode = $this->barcode_c128($code, 'A');
				break;
			}
			case 'C128B': { 				$arrcode = $this->barcode_c128($code, 'B');
				break;
			}
			case 'C128C': { 				$arrcode = $this->barcode_c128($code, 'C');
				break;
			}
			case 'EAN2': { 				$arrcode = $this->barcode_eanext($code, 2);
				break;
			}
			case 'EAN5': { 				$arrcode = $this->barcode_eanext($code, 5);
				break;
			}
			case 'EAN8': { 				$arrcode = $this->barcode_eanupc($code, 8);
				break;
			}
			case 'EAN13': { 				$arrcode = $this->barcode_eanupc($code, 13);
				break;
			}
			case 'UPCA': { 				$arrcode = $this->barcode_eanupc($code, 12);
				break;
			}
			case 'UPCE': { 				$arrcode = $this->barcode_eanupc($code, 6);
				break;
			}
			case 'MSI': { 				$arrcode = $this->barcode_msi($code, false);
				break;
			}
			case 'MSI+': { 				$arrcode = $this->barcode_msi($code, true);
				break;
			}
			case 'POSTNET': { 				$arrcode = $this->barcode_postnet($code, false);
				break;
			}
			case 'PLANET': { 				$arrcode = $this->barcode_postnet($code, true);
				break;
			}
			case 'RMS4CC': { 				$arrcode = $this->barcode_rms4cc($code, false);
				break;
			}
			case 'KIX': { 				$arrcode = $this->barcode_rms4cc($code, true);
				break;
			}
			case 'IMB': { 				$arrcode = $this->barcode_imb($code);
				break;
			}
			case 'IMBPRE': { 				$arrcode = $this->barcode_imb_pre($code);
				break;
			}
			case 'CODABAR': { 				$arrcode = $this->barcode_codabar($code);
				break;
			}
			case 'CODE11': { 				$arrcode = $this->barcode_code11($code);
				break;
			}
			case 'PHARMA': { 				$arrcode = $this->barcode_pharmacode($code);
				break;
			}
			case 'PHARMA2T': { 				$arrcode = $this->barcode_pharmacode2t($code);
				break;
			}
			default: {
	...
	protected function barcode_code93($code) {
		$chr[48] = '131112'; 		$chr[49] = '111213'; 		$chr[50] = '111312'; 		$chr[51] = '111411'; 		$chr[52] = '121113'; 		$chr[53] = '121212'; 		$chr[54] = '121311'; 		$chr[55] = '111114'; 		$chr[56] = '131211'; 		$chr[57] = '141111'; 		$chr[65] = '211113'; 		$chr[66] = '211212'; 		$chr[67] = '211311'; 		$chr[68] = '221112'; 		$chr[69] = '221211'; 		$chr[70] = '231111'; 		$chr[71] = '112113'; 		$chr[72] = '112212'; 		$chr[73] = '112311'; 		$chr[74] = '122112'; 		$chr[75] = '132111'; 		$chr[76] = '111123'; 		$chr[77] = '111222'; 		$chr[78] = '111321'; 		$chr[79] = '121122'; 		$chr[80] = '131121'; 		$chr[81] = '212112'; 		$chr[82] = '212211'; 		$chr[83] = '211122'; 		$chr[84] = '211221'; 		$chr[85] = '221121'; 		$chr[86] = '222111'; 		$chr[87] = '112122'; 		$chr[88] = '112221'; 		$chr[89] = '122121'; 		$chr[90] = '123111'; 		$chr[45] = '121131'; 		$chr[46] = '311112'; 		$chr[32] = '311211'; 		$chr[36] = '321111'; 		$chr[47] = '112131'; 		$chr[43] = '113121'; 		$chr[37] = '211131'; 		$chr[128] = '121221'; 		$chr[129] = '311121'; 		$chr[130] = '122211'; 		$chr[131] = '312111'; 		$chr[42] = '111141'; 		$code = strtoupper($code);
		$encode = array(
	...
	protected function barcode_msi($code, $checksum=false) {
		...
		$seq .= '1001'; 		$bararray = array('code' => $code, 'maxw' => 0, 'maxh' => 1, 'bcode' => array());
	...
	protected function barcode_eanupc($code, $len=13) {
		...
		$seq = '101'; 		if ($upce) {
			...
			$seq .= '01010'; 			for ($i = $half_len; $i < $len; ++$i) {
				$seq .= $codes['C'][$code{$i}];
			}
			$seq .= '101'; 		}
		$clen = strlen($seq);
	...
	protected function barcode_eanext($code, $len=5) {
		...
		$p = $parities[$len][$r];
		$seq = '1011'; 		$seq .= $codes[$p[0]][$code[0]];
		for ($i = 1; $i < $len; ++$i) {
			$seq .= '01'; 			$seq .= $codes[$p[$i]][$code{$i}];
		}
		$bararray = array('code' => $code, 'maxw' => 0, 'maxh' => 1, 'bcode' => array());
		return $this->binseq_to_array($seq, $bararray);
	}
	...
	protected function imb_crc11fcs($code_arr) {
		$genpoly = 0x0F35; 		$fcs = 0x07FF; 				$data = hexdec($code_arr[0]) << 5;
		for ($bit = 2; $bit < 8; ++$bit) {
			if (($fcs ^ $data) & 0x400) {
				$fcs = ($fcs << 1) ^ $genpoly;
			} else {
	...
	protected function imb_tables($n, $size) {
		$table = array();
		$lli = 0; 		$lui = $size - 1; 		for ($count = 0; $count < 8192; ++$count) {
			$bit_count = 0;
