...
class TCPDF_PARSER {
	...
	protected function decodeXref($startxref, $xref=array()) {
		$startxref += 4; 				$offset = $startxref + strspn($this->pdfdata, "\x00\x09\x0a\x0c\x0d\x20", $startxref);
				$obj_num = 0;
		...
		foreach ($sarr as $k => $v) {
			if (($v[0] == '/') AND ($v[1] == 'Type') AND (isset($sarr[($k +1)]) AND ($sarr[($k +1)][0] == '/') AND ($sarr[($k +1)][1] == 'XRef'))) {
				...
				if ($valid_crs AND isset($xrefcrs[1][3][0])) {
					...
					switch ($predictor) {
						case 10: { 							$ddata[$k][$j] = $row[$i];
							break;
						}
						case 11: { 							$ddata[$k][$j] = (($row[$i] + $row_left) & 0xff);
							break;
						}
						case 12: { 							$ddata[$k][$j] = (($row[$i] + $row_up) & 0xff);
							break;
						}
						case 13: { 							$ddata[$k][$j] = (($row[$i] + (($row_left + $row_up) / 2)) & 0xff);
							break;
						}
						case 14: { 														$p = ($row_left + $row_up - $row_upleft);
														$pa = abs($p - $row_left);
							$pb = abs($p - $row_up);
							$pc = abs($p - $row_upleft);
							$pmin = min($pa, $pb, $pc);
						...
						default: { 							$this->Error('Unknown PNG predictor');
							break;
						}
					}
				}
			...
			foreach ($sdata as $k => $row) {
				switch ($row[0]) {
					...
					case 1: { 												$index = $obj_num.'_'.$row[2];
												if (!isset($xref['xref'][$index])) {
														$xref['xref'][$index] = $row[1];
						}
						break;
					}
					case 2: { 																		$index = $row[1].'_0_'.$row[2];
						$xref['xref'][$index] = -1;
						break;
					}
					default: { 						break;
	...
	protected function getRawObject($offset=0) {
		$objtype = ''; 		$objval = ''; 				$offset += strspn($this->pdfdata, "\x00\x09\x0a\x0c\x0d\x20", $offset);
				$char = $this->pdfdata[$offset];
				switch ($char) {
			case '%': { 								$next = strcspn($this->pdfdata, "\r\n", $offset);
				if ($next > 0) {
					$offset += $next;
			...
			case '/': { 								$objtype = $char;
				...
				if (preg_match('/^([^\x00\x09\x0a\x0c\x0d\x20\s\x28\x29\x3c\x3e\x5b\x5d\x7b\x7d\x2f\x25]+)/', substr($this->pdfdata, $offset, 256), $matches) == 1) {
					$objval = $matches[1]; 					$offset += strlen($objval);
				}
				break;
			}
			case '(':   			case ')': { 								$objtype = $char;
				++$offset;
				$strpos = $offset;
				if ($char == '(') {
					$open_bracket = 1;
			...
			case '<':   			case '>': { 				if (isset($this->pdfdata{($offset + 1)}) AND ($this->pdfdata{($offset + 1)} == $char)) {
										$objtype = $char.$char;
					$offset += 2;
					if ($char == '<') {
												$objval = array();
