<?php
    ...
    $confirm      = optional_param('confirm', '', PARAM_ALPHANUM);       $confirmuser  = optional_param('confirmuser', 0, PARAM_INT);
    $sort         = optional_param('sort', 'name', PARAM_ALPHANUM);
    $dir          = optional_param('dir', 'ASC', PARAM_ALPHA);
    $page         = optional_param('page', 0, PARAM_INT);
    $perpage      = optional_param('perpage', 30, PARAM_INT);            $ru           = optional_param('ru', '2', PARAM_INT);                $lu           = optional_param('lu', '2', PARAM_INT);                $acl          = optional_param('acl', '0', PARAM_INT);               $suspend      = optional_param('suspend', 0, PARAM_INT);
    ...
    if ($confirmuser and confirm_sesskey()) {
    ...
    } else if ($delete and confirm_sesskey()) {                      require_capability('moodle/user:delete', $sitecontext);
        ...
        if ($confirm != md5($delete)) {
            ...
            if (delete_user($user)) {
                \core\session\manager::gc();                 redirect($returnurl);
            } else {
                \core\session\manager::gc();                 echo $OUTPUT->header();
    ...
    if (!$users) {
        ...
        if ($sort == "country") {              foreach ($users as $user) {
