...
M.core_role.init_add_assign_page = function(Y) {
    ...
    YUI().use('node', function(Y) {
        //node.delegate("change", changeRedirect, "#assign_role_category0, #assign_role_category1, #assign_role_category2");
        Y.all("#assign_role_category0, #assign_role_category1, #assign_role_category2").on("change", function(a,b,c) {
            target = Y.one(a.target);
            var targetid = target.getAttribute('id');
            console.log('#'+targetid+" change");
            YUI().use('node', 'yui2-dom', 'querystring-parse', 'querystring-stringify-simple', function(Y) {
                query = Y.QueryString.parse(window.location.search.substr(1));
                //f = Y.YUI2.util.Dom.get("changecategoryform");
                //f.submit();
//debugger;
                cate1=Y.one('#'+targetid+' option:checked');
                query.contextid = cate1.getAttribute('value');
                
                //window.location.replace(M.cfg.wwwroot + '/admin/roles/assign.php?' + Y.QueryString.stringify(query));
                window.location.href = M.cfg.wwwroot + '/admin/roles/assign.php?' + Y.QueryString.stringify(query);
            });
        });
    
        /*var category_select = Y.one('#assign_role_category0');
        category_select.on('change', function(isempty) {
            //remove.set('disabled', isempty);
            debugger;
            console.log("#assign_role_category change");
