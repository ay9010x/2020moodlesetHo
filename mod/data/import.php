...
$id              = optional_param('id', 0, PARAM_INT);  $d               = optional_param('d', 0, PARAM_INT);   $rid             = optional_param('rid', 0, PARAM_INT); $fielddelimiter  = optional_param('fielddelimiter', ',', PARAM_CLEANHTML); $fieldenclosure = optional_param('fieldenclosure', '', PARAM_CLEANHTML);   
...
if (!$formdata = $form->get_data()) {
    ...
    if (empty($readcount)) {
        ...
        while ($record = $cir->next()) {
            if ($recordid = data_add_record($data, 0)) {                  $fields = $DB->get_records('data_fields', array('dataid'=>$data->id), '', 'name, id, type');
