...
class core_admin_renderer extends plugin_renderer_base {
    ...
    protected function moodle_copyright() {
        global $CFG;
                        ...
                        $copyrighttext = '<a href="http://moodle.org/">Moodle</a> '.
                         '<a href="http://docs.moodle.org/dev/Releases" title="2016052302.00">3.1.3 (Build: 20160912)</a><br />'.
                         'Copyright &copy; 1999 onwards, Martin Dougiamas<br />'.
                         'and <a href="http://www.click-ap.com/moodleset">Click-AP Learning Technology co.,ltd.</a>.<br />'.
    ...
    protected function release_notes_link() {
        ...
        $releasenoteslink = str_replace('target="_blank"', 'onclick="this.target=\'_blank\'"', $releasenoteslink);         return $this->box($releasenoteslink, 'generalbox releasenoteslink');
    }
    ...
    public function plugins_check_table(core_plugin_manager $pluginman, $version, array $options = array()) {
        ...
        $out .= $this->output->container_end();         $out .= $this->output->container_end(); 
    ...
    public function environment_check_table($result, $environment_results) {
        global $CFG;
                ...
                $servertable = new html_table();        $servertable->head  = array(
            get_string('name'),
            get_string('info'),
            get_string('report'),
            get_string('plugin'),
        ...
        $othertable = new html_table();        $othertable->head  = array(
            get_string('info'),
            get_string('report'),
            get_string('plugin'),
            get_string('status'),
                    ...
                    if ($rec->needed = $environment_result->getNeededVersion()) {
                        ...
                        if ($environment_result->getLevel() == 'required') {
                            ...
                            $stringtouse = 'environmentrecommendinstall';
                                        ...
                                        if ($environment_result->getBypassStr() != '') {                                    $status = get_string('bypassed');
                        $warningline = true;
                    } else if ($environment_result->getRestrictStr() != '') {                           $status = get_string('restricted');
                        $errorline = true;
                    } else {
                        if ($status) {                                                                      $status = get_string('ok');
                        } else {
                            if ($environment_result->getLevel() == 'optional') {                                $status = get_string('check');
                                $warningline = true;
                            } else {                                                                            $status = get_string('check');
                                $errorline = true;
                            }
                        }
                    }
