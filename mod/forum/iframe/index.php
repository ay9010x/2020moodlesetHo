...
function refreshFrame(){
    document.getElementById('specialgraph').contentWindow.location.reload(true);
}
...
<body>
	...
	<div class="item">
		...
		<h1 class="ui header" ondblclick="show_submit_report();"><?php echo strtoupper($forum->type);?></h1>
	</div>
	<div class="item">

		<?php
			$regex_cut_array = array();
			$regex_process = preg_match_all('/\((.*?)\)/',$discussion->name, $regex_cut_array);
			$true_group_name = end(end($regex_cut_array));
			echo $true_group_name;
    ...
    <div class="MainContainer ui grid">
			<div class="eight wide column">
				<?php
				if ($forum->type == 'snifs'){
					$src="'/snifs-personal-layout/snifs-personal-layout.php?layout=group&d=".$d."&forum=".$forum->id."&course=".$course->id."'";
				}
				elseif ($forum->type == 'ipcs') {
					//$src='1438135319842.png';
					$src="'/mod/forum/skunk_iframe/IPCS/IPCS_drawing.php?user=".$USER->username."&group=".$true_group_name."&course=".$course->id."&forum=".$forum->id."'";
				}
				...
				*/?>
				<iframe name="specialgraph" class="frameBox" id="specialgraph" src=<?php echo $src; ?> scrolling="yes"></iframe>
			</div>
