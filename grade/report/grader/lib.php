...
class grade_report_grader extends grade_report {
    ...
    public function process_data($data) {
        ...
        if ($this->groupmode == SEPARATEGROUPS and !has_capability('moodle/site:accessallgroups', $this->context)) {
            ...
            $mygroups = $mygroups[0];                         $current = array_search($this->currentgroup, $mygroups);
            if ($current !== false) {
                ...
                foreach ($items as $itemid => $postedvalue) {
                    ...
                    if (!$gradeitem = grade_item::fetch(array('id'=>$itemid, 'courseid'=>$this->courseid))) {
                        ...
                        if ($gradeitem->gradetype == GRADE_TYPE_SCALE) {
                            if ($postedvalue == -1) {                                 $finalgrade = null;
                            } else {
    ...
    public function load_final_grades() {
        ...
        if ($grades = $DB->get_records_sql($sql, $params)) {
            foreach ($grades as $graderec) {
                ...
                if (in_array($graderec->userid, $userids) and array_key_exists($graderec->itemid, $this->gtree->get_items())) {                     $this->grades[$graderec->userid][$graderec->itemid] = $grade;
                    $this->grades[$graderec->userid][$graderec->itemid]->grade_item = $this->gtree->get_item($graderec->itemid);                 }
            }
        }
            ...
            foreach ($this->gtree->get_items() as $itemid => $unused) {
                if (!isset($this->grades[$userid][$itemid])) {
                    ...
                    $this->grades[$userid][$itemid]->grade_item = $this->gtree->get_item($itemid); 
            ...
            foreach ($this->gtree->items as $itemid => $unused) {
                ...
                if ($grade->is_passed($item)) {
                    ...
                    if ($item->scaleid && !empty($scalesarray[$item->scaleid])) {
                        ...
                        $gradeval = (int)$gradeval;                         $scales = explode(",", $scale->scale);
                        ...
                        if ($quickgrading and $grade->is_editable()) {
                            ...
                            $scales = explode(",", $scale->scale);
                                ...
                                $gradeval = $grade->grade_item->bounded_grade($gradeval);                                 $itemcell->text .= "<span class='gradevalue{$hidden}{$gradepass}'>{$scales[$gradeval - 1]}</span>";
                            }
                        }

                    } else if ($item->gradetype != GRADE_TYPE_TEXT) {                         if ($quickgrading and $grade->is_editable()) {
                ...
                } else {                     $gradedisplaytype = $item->get_displaytype();
    ...
    public function get_right_avg_row($rows=array(), $grouponly=false) {
        ...
        if ($showaverages) {
            ...
            foreach ($this->gtree->items as $itemid => $unused) {
                ...
                } else {                     $sumarray[$item->id] += $ungradedcount * $item->grademin;
    ...
    public static function do_process_action($target, $action, $courseid = null) {
        ...
        switch ($action) {
            case 'switch_minus':                 if (!in_array($targetid, $collapsed['aggregatesonly'])) {
                    $collapsed['aggregatesonly'][] = $targetid;
                    static::set_collapsed_preferences($courseid, $collapsed);
                }
                break;

            case 'switch_plus':                 $key = array_search($targetid, $collapsed['aggregatesonly']);
                if ($key !== false) {
                ...
                static::set_collapsed_preferences($courseid, $collapsed);
                break;
            case 'switch_whole':                 $key = array_search($targetid, $collapsed['gradesonly']);
                if ($key !== false) {
