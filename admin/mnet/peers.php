...
$simpleform = new mnet_simple_host_form(); $reviewform = null; 
if ($formdata = $simpleform->get_data()) {
    ...
    $mnet_peer->bootstrap($formdata->wwwroot, null, $application);
        $reviewform = new mnet_review_host_form(null, array('peer' => $mnet_peer));     $formdata->oldpublickey = $mnet_peer->public_key;     $reviewform->set_data($mnet_peer);
    echo $OUTPUT->header();
    ...
    echo $OUTPUT->footer();
    exit;
} else if ($simpleform->is_submitted()) {     $noreviewform = true;
}
...
if (!empty($hostid)) {
    ...
    if ($hostid != $CFG->mnet_all_hosts_id) {
        ...
        $reviewform = new mnet_review_host_form(null, array('peer' => $mnet_peer));         $mnet_peer->oldpublickey = $mnet_peer->public_key;         $reviewform->set_data((object)$mnet_peer);
        echo $OUTPUT->box_start();
        $reviewform->display();
        echo $OUTPUT->box_end();
    } else {
...
} else if ($reviewform->is_submitted()) {     echo $OUTPUT->header();
    echo $OUTPUT->box_start();
    $reviewform->display();
    echo $OUTPUT->box_end();
    echo $OUTPUT->footer();
