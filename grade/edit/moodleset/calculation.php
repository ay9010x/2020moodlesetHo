...
$url = new moodle_url('/grade/edit/moodleset/calculation.php', array('id'=>$id, 'courseid'=>$courseid));
...
} elseif (!empty($section) AND $section='idnumbers' AND !empty($idnumbers)) {         foreach ($idnumbers as $giid => $value) {
...
$gtree = new grade_moodleset($course->id, false, false);
...
$mform->display();
echo '
<form class="mform" id="mform2" method="post" action="' . $CFG->wwwroot . '/grade/edit/moodleset/calculation.php?courseid='.$courseid.'&amp;id='.$id.'">
    ...
    <fieldset id="idnumbers" class="clearfix">
        ...
        <div class="fcontainer clearfix">
            <ul>
            ' . get_grade_moodleset($gtree, $gtree->top_element, $id, $errors) . '
            </ul>
        </div>
    </fieldset>
...
function get_grade_moodleset(&$gtree, $element, $current_itemid=null, $errors=null) {
    global $CFG;
    ...
    if ($type != 'category') {
        ...
        foreach($element['children'] as $child_el) {
            $return_string .= get_grade_moodleset($gtree, $child_el, $current_itemid, $errors);
        }
        if ($last) {
            $return_string .= get_grade_moodleset($gtree, $last, $current_itemid, $errors);
        }
        $return_string .= '</ul></li>'."\n";
    }
