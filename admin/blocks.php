<?php
    ...
    if (!empty($hide) && confirm_sesskey()) {
        ...
        $DB->set_field('block', 'visible', '0', array('id'=>$block->id));              add_to_config_log('block_visibility', $block->visible, '0', $block->name);
    ...
    if (!empty($show) && confirm_sesskey() ) {
        ...
        $DB->set_field('block', 'visible', '1', array('id'=>$block->id));              add_to_config_log('block_visibility', $block->visible, '1', $block->name);
        core_plugin_manager::reset_caches();
        admin_get_root(true, false);      }
    ...
    foreach ($blocknames as $blockid=>$strblockname) {
        ...
        $settings = '';         if ($blockobject and $blockobject->has_config()) {
