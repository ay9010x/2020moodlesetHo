...
class ADODB_DataDict {
	...
	var $renameColumn = 'ALTER TABLE %s RENAME COLUMN %s TO %s';		var $nameRegex = '\w';
	var $nameRegexBrackets = 'a-zA-Z0-9_\(\)';
	var $schema = false;
	var $serverInfo = array();
	var $autoIncrement = false;
	var $dataProvider;
	var $invalidResizeTypes4 = array('CLOB','BLOB','TEXT','DATE','TIME'); 	var $blobSize = 100; 								
	...
	function _GenFields($flds,$widespacing=false)
	{
		...
		foreach($flds as $fld) {
			...
			if ($ty == 'X' || $ty == 'X2' || $ty == 'B') $fnotnull = false; 
