...
class ADODB_SAPDB extends ADODB_odbc {
	...
	var $fmtDate = "'Y-m-d'";		var $fmtTimeStamp = "'Y-m-d H:i:s'"; 	var $hasInsertId = true;
 	...
 	function MetaIndexes ($table, $primary = FALSE, $owner = false)
	{
		...
		if ($primary) {
			$indexes['SYSPRIMARYKEYINDEX'] = array(
					'unique' => True,						'columns' => $this->GetCol("SELECT columnname FROM COLUMNS WHERE tablename=$table AND mode='KEY' ORDER BY pos"),
				);
		}
        return $indexes;
	}
