...
class PHPExcel_Calculation
{
        ...
        if (count(self::$validLocaleLanguages) == 1) {
            ...
            self::$localeBoolean = array('TRUE' => 'TRUE', 'FALSE' => 'FALSE', 'NULL' => 'NULL');
                ...
                foreach ($localeFunctions as $localeFunction) {
                    list($localeFunction) = explode('##', $localeFunction);                        if (strpos($localeFunction, '=') !== false) {
                ...
                if (file_exists($configFile)) {
                    ...
                    foreach ($localeSettings as $localeSetting) {
                        list($localeSetting) = explode('##', $localeSetting);                            if (strpos($localeSetting, '=') !== false) {
        ...
        private function _parseFormula($formula, PHPExcel_Cell $pCell = null)
    {
        ...
        $output = array();
        $expectingOperator = false;                                                                                $expectingOperand = false;                                                                                                while (true) {
            $opCharacter = $formula{$index};                if ((isset(self::$comparisonOperators[$opCharacter])) && (strlen($formula) > $index) && (isset(self::$comparisonOperators[$formula{$index+1}]))) {
                $opCharacter .= $formula{++$index};
            }

                        $isOperandOrFunction = preg_match($regexpMatchString, substr($formula, $index), $match);

            if ($opCharacter == '-' && !$expectingOperator) {                                $stack->push('Unary Operator', '~');                                            ++$index;                                                                } elseif ($opCharacter == '%' && $expectingOperator) {
                $stack->push('Unary Operator', '%');                                            ++$index;
            } elseif ($opCharacter == '+' && !$expectingOperator) {                            ++$index;                                                                } elseif ((($opCharacter == '~') || ($opCharacter == '|')) && (!$isOperandOrFunction)) {                    return $this->raiseFormulaError("Formula Error: Illegal character '~'");                
            } elseif ((isset(self::$operators[$opCharacter]) or $isOperandOrFunction) && $expectingOperator) {                    while ($stack->count() > 0 &&
                    ($o2 = $stack->last()) &&
                    isset(self::$operators[$o2['value']]) &&
                    @(self::$operatorAssociativity[$opCharacter] ? self::$operatorPrecedence[$opCharacter] < self::$operatorPrecedence[$o2['value']] : self::$operatorPrecedence[$opCharacter] <= self::$operatorPrecedence[$o2['value']])) {
                    $output[] = $stack->pop();                                                }
                $stack->push('Binary Operator', $opCharacter);                    ++$index;
                $expectingOperator = false;

            } elseif ($opCharacter == ')' && $expectingOperator) {                            $expectingOperand = false;
                while (($o2 = $stack->pop()) && $o2['value'] != '(') {                            if ($o2 === null) {
                ...
                $d = $stack->last(2);
                if (preg_match('/^'.self::CALCULATION_REGEXP_FUNCTION.'$/i', $d['value'], $matches)) {                        $functionName = $matches[1];                                                            $d = $stack->pop();
                    $argumentCount = $d['value'];                            $output[] = $d;                                            $output[] = $stack->pop();                                if (isset(self::$controlFunctions[$functionName])) {
            ...
            } elseif ($opCharacter == ',') {                            while (($o2 = $stack->pop()) && $o2['value'] != '(') {                            if ($o2 === null) {
                ...
                $d = $stack->pop();
                $stack->push($d['type'], ++$d['value'], $d['reference']);                    $stack->push('Brace', '(');                    $expectingOperator = false;
            ...
            } elseif ($isOperandOrFunction && !$expectingOperator) {                    $expectingOperator = true;
                ...
                if (preg_match('/^'.self::CALCULATION_REGEXP_FUNCTION.'$/i', $val, $matches)) {
                    ...
                    if (isset(self::$PHPExcelFunctions[strtoupper($matches[1])]) || isset(self::$controlFunctions[strtoupper($matches[1])])) {                            $stack->push('Function', strtoupper($val));
                        ...
                        $stack->push('Brace', '(');
                    } else {                            $output[] = array('type' => 'Value', 'value' => $matches[1], 'reference' => null);
                    }
                    ...
                    if ($testPrevOp['value'] == ':') {
                        ...
                        if ((is_integer($startRowColRef)) && (ctype_digit($val)) &&
                            ($startRowColRef <= 1048576) && ($val <= 1048576)) {
                                                        $endRowColRef = ($pCellParent !== null) ? $pCellParent->getHighestColumn() : 'XFD';                                $output[count($output)-1]['value'] = $rangeWS1.'A'.$startRowColRef;
                            $val = $rangeWS2.$endRowColRef.$val;
                        } elseif ((ctype_alpha($startRowColRef)) && (ctype_alpha($val)) &&
                            (strlen($startRowColRef) <= 3) && (strlen($val) <= 3)) {
                                                        $endRowColRef = ($pCellParent !== null) ? $pCellParent->getHighestRow() : 1048576;                                    $output[count($output)-1]['value'] = $rangeWS1.strtoupper($startRowColRef).'1';
            ...
            } else {                    return $this->raiseFormulaError("Formula Error: An unexpected error occured");
            }
        ...
        while (($op = $stack->pop()) !== null) {                if ((is_array($op) && $op['value'] == '(') || ($op === '(')) {
            ...
            $token = $tokenData['value'];
                ...
                $operand2 = self::dataTestReference($operand2Data);
                    ...
                    $this->_debugLog->writeDebugLog('Evaluating ', $this->showValue($operand1), ' ', $token, ' ', $this->showValue($operand2));
                                ...
                                switch ($token) {
                                        case '>':                                case '<':                                case '>=':                                case '<=':                                case '=':                                case '<>':                                    $this->executeBinaryComparisonOperation($cellID, $operand1, $operand2, $token, $stack);
                        break;
                                        case ':':                                    $sheet1 = $sheet2 = '';
                        ...
                        break;
                    case '+':                                    $this->executeNumericBinaryOperation($cellID, $operand1, $operand2, $token, 'plusEquals', $stack);
                        break;
                    case '-':                                    $this->executeNumericBinaryOperation($cellID, $operand1, $operand2, $token, 'minusEquals', $stack);
                        break;
                    case '*':                                    $this->executeNumericBinaryOperation($cellID, $operand1, $operand2, $token, 'arrayTimesEquals', $stack);
                        break;
                    case '/':                                    $this->executeNumericBinaryOperation($cellID, $operand1, $operand2, $token, 'arrayRightDivide', $stack);
                        break;
                    case '^':                                    $this->executeNumericBinaryOperation($cellID, $operand1, $operand2, $token, 'power', $stack);
                        break;
                    case '&':                                                                                                            if (is_bool($operand1)) {
                ...
                if ((isset(self::$PHPExcelFunctions[$functionName])) || (isset(self::$controlFunctions[$functionName]))) {                        if (isset(self::$PHPExcelFunctions[$functionName])) {
                        $functionCall = self::$PHPExcelFunctions[$functionName]['functionCall'];
