...
function scorm_seq_overall ($scoid, $userid, $request, $attempt) {
    ...
    if ($seq->navigation) {
        ...
        if ($seq->sequencing != null) {
            ...
            if ($seq->sequencing == 'exit') {                 return 'true';
            }
        }
    ...
    switch ($request) {
            ...
            if (!empty($seq->currentactivity)) {
                ...
                if ($sco->parent != '/') {
                    if ($parentsco = scorm_get_parent($sco)) {
                        ...
                        if (isset($parentsco->flow) && ($parentsco->flow == true)) {                                                         if (scorm_seq_is('active', $sco->id, $userid)) {
        ...
        case 'suspendall_':
            if (!empty($seq->currentactivity)) {
                ...
                $seq->exception = 'NB.2.1-2';             }
        break;
        default:             if ($targetsco = $DB->get_record('scorm_scoes', array('scorm' => $sco->scorm, 'identifier' => $request))) {
                if ($targetsco->parent != '/') {
                    $seq->target = $request;
                } else {
                                ...
                                if ($activitypath = array_slice($ancestors, 0, $commonpos)) {
                                    foreach ($activitypath as $activity) {
                                        if (scorm_seq_is('active', $activity->id, $userid) &&
                                            ...
                                            $seq->sequencing = null;
                                            $seq->target = null;
                                            $seq->exception = 'NB.2.1-8';                                             return $seq;
                                        }
                                    }
                                } else {
                                    $seq->navigation = false;
...
function scorm_seq_termination ($seq, $userid) {
    ...
    switch ($seq->termination) {
        case 'exit':
            ...
            do {
                $exit = false;                $seq = scorm_seq_post_cond_rules($seq, $userid);
...
function scorm_seq_measure_rollup($sco, $userid, $attempt = 0) {
    ...
    $totalmeasure = 0;     $valid = false;     $countedmeasures = 0;     $targetobjective = null;
    $objectives = $DB->get_records('scorm_seq_objective', array('scoid' => $sco->id));

    foreach ($objectives as $objective) {
        if ($objective->primaryobj == true) {             $targetobjective = $objective;
            break;
        }
        ...
        if (!empty ($children)) {
            foreach ($children as $child) {
                ...
                if (!isset($child->tracked) || ($child->tracked == 1)) {
                    $rolledupobjective = null;                    $objectives = $DB->get_records('scorm_seq_objective', array('scoid' => $child->id));
                    foreach ($objectives as $objective) {
                        if ($objective->primaryobj == true) {                            $rolledupobjective = $objective;
                            break;
                        }
                    }
...
function scorm_seq_objective_rollup_rules($sco, $userid, $attempt = 0) {
    ...
    if ($targetobjective != null) {
        ...
        if (scorm_seq_rollup_rule_check($sco, $userid, 'notsatisfied')) {            scorm_seq_set('objectiveprogressstatus', $sco->id, $userid, $attempt);
            scorm_seq_set('objectivesatisfiedstatus', $sco->id, $userid, $attempt, false);
        }
        if (scorm_seq_rollup_rule_check($sco, $userid, 'satisfied')) {            scorm_seq_set('objectiveprogressstatus', $sco->id, $userid, $attempt);
            scorm_seq_set('objectivesatisfiedstatus', $sco->id, $userid, $attempt);
        }
    ...
    if ($rolluprules = $DB->get_record('scorm_seq_rolluprule', array('scoid' => $sco->id, 'action' => $action))) {
        ...
        foreach ($rolluprules as $rolluprule) {
            ...
            switch ($rolluprule->childactivityset) {
                ...
                case 'atleastcount':
                    foreach ($childrenbag as $itm) {                        $cont = 0;
                        if ($itm === true) {
                            $cont++;
                        }
                        if ($cont >= $rolluprule->minimumcount) {
