...
class Less_Functions{
	...
	public function hsla($h, $s, $l, $a){
		...
		$h = fmod(self::number($h), 360) / 360; 		$s = self::clamp(self::number($s));
	...
	public function extract($values, $index ){
		$index = (int)$index->value - 1; 						if( property_exists($values,'value') && is_array($values->value) ){
	...
	public function colorBlend( $mode, $color1, $color2 ){
		$ab = $color1->alpha;			$as = $color2->alpha;			$r = array();			
		$ar = $as + $ab * (1 - $as);
		for( $i = 0; $i < 3; $i++ ){
			$cb = $color1->rgb[$i] / 255;
			$cs = $color2->rgb[$i] / 255;
