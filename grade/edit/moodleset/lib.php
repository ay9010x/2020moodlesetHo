...
class grade_edit_moodleset {
    ...
    public function __construct($gtree, $moving=false, $gpr) {
        ...
        $this->columns = array(grade_edit_moodleset_column::factory('name', array('deepest_level' => $this->deepest_level)));

        if ($this->uses_weight) {
            $this->columns[] = grade_edit_moodleset_column::factory('weight', array('adv' => 'weight'));
        }

                
        if ($this->deepest_level > 1) {
            $this->columns[] = grade_edit_moodleset_column::factory('select');
        }

        $this->table = new html_table();
        $this->table->id = "grade_edit_moodleset_table";
        ...
        $rowcount = 0;
        $this->table->data = $this->build_html_moodleset($this->gtree->top_element, true, array(), 0, $rowcount);
    }

    
    public function build_html_moodleset($element, $totals, $parents, $level, &$row_count) {
            ...
            foreach($element['children'] as $child_el) {
                ...
                $child_row_count = 0;
                    ...
                    $html_children[] = $moveto;
                                        ...
                                        $category_total_item = $this->build_html_moodleset($child_el, $totals, $newparents, $level, $child_row_count);
                    if (!$aggregation_position) {
                        $html_children = array_merge($html_children, $category_total_item);
                    }
                } else {
                    $html_children = array_merge($html_children, $this->build_html_moodleset($child_el, $totals, $newparents, $level, $child_row_count));
                    if (!empty($moveto)) {
                        $html_children[] = $moveto;
                    }
            ...
            foreach ($this->columns as $column) {
                if (!($this->moving && $column->hide_when_moving)) {
                    $row->cells[] = $column->get_category_cell($category, $levelclass, array('id' => $id,
                        'name' => $object->name, 'level' => $level, 'eid' => $eid));                 }
            }
            ...
            foreach ($this->columns as $column) {
                if (!($this->moving && $column->hide_when_moving)) {
                    ...
                    $gradeitemrow->cells[] = $column->get_item_cell($item, array('id' => $id, 'name' => $object->name,
                        'level' => $level, 'element' => $element, 'eid' => $eid,'itemtype' => $object->itemtype)
                        ); 
                                        }
            }
    ...
    static function get_weight_input($item) {
        ...
        if (!is_object($item) || get_class($item) !== 'grade_item') {
            throw new Exception('grade_edit_moodleset::get_weight_input($item) was given a variable that is not of the required type (grade_item object)');
            return false;
        }
        ...
        if ($aggcoef == 'aggregationcoefweight' || $aggcoef == 'aggregationcoef' || $aggcoef == 'aggregationcoefextraweight') {
            return '<label class="accesshide" for="weight_'.$item->id.'">'.
                ...
                value="'.grade_edit_moodleset::format_number($item->aggregationcoef).'" />%';
            ...
            $checkbox .= html_writer::empty_tag('input', array('name' => $checkboxname, 'type' => 'checkbox', 
                'value' => 1, 'id' => $checkboxname, 'class' => 'weightoverride',
            ...
            $input = html_writer::empty_tag(
                ...
                array(
                    ...
                    'id' =>     $name,
                    'name' =>   $name,
                    'value' =>  grade_edit_moodleset::format_number($item->aggregationcoef2 * 100.0),
                )
            );
                        $input .= '-%-';

                        $str .= $hiddenlabel . $input;
        }
...
abstract class grade_edit_moodleset_column {
    public $forced;
    public $hidden;
    ...
    public static function factory($name, $params=array()) {
        $class_name = "grade_edit_moodleset_column_$name";
    ...
    public function __construct() {
        ...
        if (preg_match('/^grade_edit_moodleset_column_(\w*)$/', get_class($this), $matches)) {
...
class grade_edit_moodleset_column_name extends grade_edit_moodleset_column {
    public $forced = false;
    public $hidden = false;
    ...
    public function __construct($params) {
        if (empty($params['deepest_level'])) {
            throw new Exception('Tried to instantiate a grade_edit_moodleset_column_name object without the "deepest_level" param!');
        }
    ...
    public function get_category_cell($category, $levelclass, $params) {
        ...
        if (empty($params['name']) || empty($params['level'])) {
            throw new Exception('Array key (name or level) missing from 3rd param of grade_edit_moodleset_column_name::get_category_cell($category, $levelclass, $params)');
        }
    ...
    public function get_item_cell($item, $params) {
        ...
        if (empty($params['element']) || empty($params['name']) || empty($params['level'])) {
            throw new Exception('Array key (name, level or element) missing from 2nd param of grade_edit_moodleset_column_name::get_item_cell($item, $params)');
        }
...
class grade_edit_moodleset_column_weight extends grade_edit_moodleset_column {
    ...
    public function get_category_cell($category, $levelclass, $params) {
        ...
        $item = $category->get_grade_item();
        $categorycell = parent::get_category_cell($category, $levelclass, $params);
        $categorycell->text = grade_edit_moodleset::get_weight_input($item);
        return $categorycell;
    }
    ...
    public function get_item_cell($item, $params) {
        ...
        if (empty($params['element'])) {
            throw new Exception('Array key (element) missing from 2nd param of grade_edit_moodleset_column_weightorextracredit::get_item_cell($item, $params)');
        }
        $itemcell = parent::get_item_cell($item, $params);
        $itemcell->text = '&nbsp;';
        ...
        if (!in_array($object->itemtype, array('courseitem', 'categoryitem', 'category'))
            ...
            $itemcell->text = grade_edit_moodleset::get_weight_input($item);
        }
...
class grade_edit_moodleset_column_range extends grade_edit_moodleset_column {
...
class grade_edit_moodleset_column_actions extends grade_edit_moodleset_column {
    ...
    public function get_category_cell($category, $levelclass, $params) {
        ...
        if (empty($params['actions'])) {
            throw new Exception('Array key (actions) missing from 3rd param of grade_edit_moodleset_column_actions::get_category_actions($category, $levelclass, $params)');
        }
    ...
    public function get_item_cell($item, $params) {
        if (empty($params['actions'])) {
            throw new Exception('Array key (actions) missing from 2nd param of grade_edit_moodleset_column_actions::get_item_cell($item, $params)');
        }
        $itemcell = parent::get_item_cell($item, $params);
...
class grade_edit_moodleset_column_select extends grade_edit_moodleset_column {
    ...
    public function get_category_cell($category, $levelclass, $params) {
        global $OUTPUT;
        if (empty($params['eid'])) {
            throw new Exception('Array key (eid) missing from 3rd param of grade_edit_moodleset_column_select::get_category_cell($category, $levelclass, $params)');
        }
