...
$cmid       = required_param('cmid', PARAM_INT);            $id         = required_param('id', PARAM_INT);              $edit       = optional_param('edit', false, PARAM_BOOL);    $delete     = optional_param('delete', false, PARAM_BOOL);  $confirm    = optional_param('confirm', false, PARAM_BOOL); $assess     = optional_param('assess', false, PARAM_BOOL);  
...
if ($id) {     $example = $workshop->get_example_by_id($id);
} else {     require_capability('mod/workshop:manageexamples', $workshop->context);
...
if ($edit and $canmanage) {
    ...
    if ($mform->is_cancelled()) {
        ...
        $formdata->content            = '';                  $formdata->contentformat      = FORMAT_HTML;         $formdata->contenttrust       = 0;                   if (is_null($example->id)) {
...
echo $output->container_end(); echo $output->footer();
