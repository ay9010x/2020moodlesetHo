...
function xmldb_block_course_menu_install($oldversion=0) {
    global $CFG, $DB;  
       
    $dbman = $DB->get_manager();

    $instances = new stdClass();
    $instances->blockname  = 'course_menu';
    $instances->parentcontextid  = '1';
    $instances->showinsubcontexts  = '1';
    $instances->pagetypepattern  = '*';
    $instances->subpagepattern = '';
    $instances->defaultregion  = 'side-pre';
    ...
    if (!in_array('course_menu', $undeletableblocktypes)) {
        set_config('undeletableblocktypes', $CFG->undeletableblocktypes.',course_menu');
    }
    ...
    while($i<=19){
        ...
        switch($i){
            case 1:
                $teacher['name'] = '課程資訊';
                $teacher['ename'] = 'Information';				// by YCJ
                $teacher['parent'] = 0;
                $teacher['url'] = '';
                $teacher['params'] = '';
