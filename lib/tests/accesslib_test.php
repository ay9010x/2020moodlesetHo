...
class core_accesslib_testcase extends advanced_testcase {
    ...
    public function test_assign_capability() {
        ...
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'moodle/backup:backupcourse')));         $this->assertFalse($DB->record_exists('role_capabilities', array('contextid'=>$syscontext->id, 'roleid'=>$student->id, 'capability'=>'moodle/backup:backupcourse')));
    ...
    public function test_unassign_capability() {
        ...
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'moodle/backup:backupcourse')));         assign_capability('moodle/backup:backupcourse', CAP_ALLOW, $manager->id, $frontcontext->id);
    ...
    public function test_get_roles_with_capability() {
        ...
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'moodle/backup:backupcourse')));         $DB->delete_records('role_capabilities', array('capability'=>'moodle/backup:backupcourse'));
    ...
    public function test_get_role_archetypes() {
        ...
        $this->assertCount(8, $archetypes);         foreach ($archetypes as $k => $v) {
            $this->assertSame($k, $v);
        }
    }
    ...
    public function test_role_fix_names() {
        ...
        $renames = $DB->get_records_menu('role_names', array('contextid'=>$coursecontext->id), '', 'roleid, name');
                        ...
                        $roles = $allroles;
        unset($roles[$student->id]);         $rolenames = array();
        foreach ($roles as $role) {
    ...
    public function test_get_overridable_roles() {
        ...
        $teacherename = (object)array('roleid'=>$teacherrole->id, 'name'=>'Učitel', 'contextid'=>$coursecontext->id);
        $DB->insert_record('role_names', $teacherename);
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'moodle/backup:backupcourse')));         assign_capability('moodle/backup:backupcourse', CAP_PROHIBIT, $teacherrole->id, $coursecontext->id);
    ...
    public function test_get_user_roles_in_course() {
        ...
        $roleids = explode(',', $CFG->profileroles);         $this->assertTrue(in_array($teacherrole->id, $roleids));
    ...
    public function test_permission_evaluation() {
        ...
        for ($i=0; $i<5; $i++) {
            ...
            context_block::instance($bi->id);
                ...
                $count = 1;         $count += $DB->count_records('user', array('deleted'=>0));
        ...
        $this->assertTrue(has_capability('moodle/course:update', $blockcontext));
                ...
                $DB->delete_records('cache_flags', array());         $userid = $testusers[5];
        ...
        foreach ($userids as $userid) {                         $rcount = rand(0, 10);
            for ($j=0; $j<$rcount; $j++) {
        ...
        $DB->delete_records('cache_flags', array());
        $context->delete();         $dirty = get_cache_flags('accesslib/dirtycontexts', time()-2);
        $this->assertTrue(isset($dirty[$context->path]));
    ...
    public function test_update_capabilities() {
        ...
        $this->assertFalse(isset($existingcaps['moodle/site:restore']));                 $this->assertTrue(isset($existingcaps['moodle/restore:restorecourse']));         $this->assertTrue(isset($existingcaps['moodle/site:sendmessage']));              $this->assertTrue(isset($existingcaps['moodle/backup:backupcourse']));
        $this->assertTrue(isset($existingcaps['moodle/backup:backupsection']));          $this->assertTrue(isset($existingcaps['moodle/site:approvecourse']));            $this->assertTrue(isset($existingcaps['moodle/course:manageactivities']));
        $this->assertTrue(isset($existingcaps['mod/page:addinstance']));         
