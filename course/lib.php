 ...
 */
function course_add_cm_to_section($courseorid, $cmid, $sectionnum, $beforemod = null, $aftermod = null) {
    global $DB, $COURSE;
    ...
    } else if ($aftermod && ($key = array_keys($modarray, $aftermod))) { /* by Mary Chen , for assign pattern*/
        $insertarray = array($aftermod, $cmid);
        array_splice($modarray, $key[0], 1, $insertarray);
        $newsequence = implode(",", $modarray);
    } else {
 ...
 * @return bool
 */
function include_course_ajax($course, $usedmodules = array(), $enabledmodules = null, $config = null, $dragdrop = null) {   // by YCJ
    global $CFG, $PAGE, $SITE;
