...
$id = required_param('id', PARAM_INT); $userid = required_param('user', PARAM_INT); $scoid = required_param('scoid', PARAM_INT); $attempt = optional_param('attempt', 1, PARAM_INT); $download = optional_param('download', '', PARAM_ALPHA);
...
foreach ($trackdata as $element => $value) {
    if (substr($element, 0, 3) == 'cmi') {
        ...
        if (stristr($element, '.id') !== false) {
            ...
            $string = "trackresult";
        } else if (stristr($element, '.student_response') !== false or             stristr($element, '.learner_response') !== false) {             $string = "trackresponse";
