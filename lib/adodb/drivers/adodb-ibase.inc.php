...
class ADODB_ibase extends ADOConnection {
	...
	var $replaceQuote = "''"; 	var $ibase_datefmt = '%Y-%m-%d'; 	var $fmtDate = "'Y-m-d'";
	...
	var $sysTimeStamp = "CURRENT_TIMESTAMP"; 	var $ansiOuter = true;
		...
		function _connect($argHostname, $argUsername, $argPassword, $argDatabasename,$persist=false)
	{
		...
		if ($this->dialect != 1) { 			$this->replaceQuote = "''";
		}
	...
	function BeginTrans()
	{
		...
		$this->_transactionID = $this->_connectionID;		return $this->_transactionID;
	}
	...
	global $ADODB_COUNTRECS;
		...
		if ($this->_logsql) {
			...
			$ADODB_COUNTRECS = true; 			$ret = ADOConnection::_Execute($sql,$inputarr);
		...
		function RowLock($tables,$where,$col=false)
	{
		...
		$this->Execute("UPDATE $table SET $col=$col WHERE $where "); 		return 1;
	}
			...
			function _query($sql,$iarr=false)
	{
		...
		if (is_array($sql)) {
			...
			$sql = $sql[1];
			if (is_array($iarr)) {
				if  (ADODB_PHPVER >= 0x4050) { 					if ( !isset($iarr[0]) ) $iarr[0] = ''; 					$fnarr = array_merge( array($sql) , $iarr);
			...
			if (is_array($iarr)) {
				if (ADODB_PHPVER >= 0x4050) { 					if (sizeof($iarr) == 0) $iarr[0] = ''; 					$fnarr = array_merge( array($conn,$sql) , $iarr);
					$ret = call_user_func_array($fn,$fnarr);
				} else {
					switch(sizeof($iarr)) {
			...
			function _BlobDecode_old( $blob )
	{
		...
		$realblob = ibase_blob_get( $blobid,$this->maxblobsize); 		while($string = ibase_blob_get($blobid, 8192)){
			$realblob .= $string;
		}
