...
$id         = required_param('id', PARAM_INT);                 $action     = optional_param('action', '', PARAM_ALPHA);
$attemptids = optional_param_array('attemptid', array(), PARAM_INT); $notify     = optional_param('notify', '', PARAM_ALPHA);
...
if (data_submitted() && is_enrolled($context, NULL, 'mod/choice:choose') && confirm_sesskey()) {
    ...
    if (has_capability('mod/choice:deleteresponses', $context) && $action == 'delete') {
                choice_delete_responses($attemptids, $choice, $cm, $course);         redirect("view.php?id=$cm->id");
    }
