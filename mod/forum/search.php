...
$id = required_param('id', PARAM_INT);                  $search = trim(optional_param('search', '', PARAM_NOTAGS));  $page = optional_param('page', 0, PARAM_INT);   $perpage = optional_param('perpage', 10, PARAM_INT);   $showform = optional_param('showform', 0, PARAM_INT);   
$user    = trim(optional_param('user', '', PARAM_NOTAGS));    $userid  = trim(optional_param('userid', 0, PARAM_INT));      $forumid = trim(optional_param('forumid', 0, PARAM_INT));      $subject = trim(optional_param('subject', '', PARAM_NOTAGS)); $phrase  = trim(optional_param('phrase', '', PARAM_NOTAGS));  $words   = trim(optional_param('words', '', PARAM_NOTAGS));   $fullwords = trim(optional_param('fullwords', '', PARAM_NOTAGS)); $notwords = trim(optional_param('notwords', '', PARAM_NOTAGS));   
$timefromrestrict = optional_param('timefromrestrict', 0, PARAM_INT); $fromday = optional_param('fromday', 0, PARAM_INT);      $frommonth = optional_param('frommonth', 0, PARAM_INT);      $fromyear = optional_param('fromyear', 0, PARAM_INT);      $fromhour = optional_param('fromhour', 0, PARAM_INT);      $fromminute = optional_param('fromminute', 0, PARAM_INT);      if ($timefromrestrict) {
...
$timetorestrict = optional_param('timetorestrict', 0, PARAM_INT); $today = optional_param('today', 0, PARAM_INT);      $tomonth = optional_param('tomonth', 0, PARAM_INT);      $toyear = optional_param('toyear', 0, PARAM_INT);      $tohour = optional_param('tohour', 0, PARAM_INT);      $tominute = optional_param('tominute', 0, PARAM_INT);      if ($timetorestrict) {
    $calendartype = \core_calendar\type_factory::get_calendar_instance();
...
$PAGE->set_url($FULLME); 
if (empty($search)) {       if (!empty($words)) {
        $search .= ' '.$words;
    }
    if (!empty($userid)) {
...
$searchterms = explode(' ', $strippedsearch);    foreach ($searchterms as $key => $searchterm) {
