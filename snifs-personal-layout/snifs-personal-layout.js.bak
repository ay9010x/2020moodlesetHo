...
function create_Teamtable(data, _team) {
    ...
    for (var i = 0; i < number_of_rows; i++) {
        ...
        table_body += '</td>';
        table_body += '<td>';
        table_body += '<a team="' + _team + '" word="' + data[i][1] + '" teamcount="' + data[i][2] + '" href="/mod/hsuforum/search.php?id=73&words=' + encodeURI(data[i][1]) + '&subject=' + encodeURI(data[0][0]) + '組討論區" target="search_discuss">' +
            '<i class="circular search link icon"></i>' +
            '</a>';
        table_body += '</td>';
        table_body += '</tr>';
