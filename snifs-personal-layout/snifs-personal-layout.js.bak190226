...
function create_Wordtable(data) {
    ...
    var table_body = '<thead><tr><th colspan = "4">搜尋[' +
        '<a class="search_word" user="ALL" href="/mod/hsuforum/search.php?id=73&words=' + encodeURI(data[0][0]) + '" target="search_discuss" >' +
        data[0][0] +
        '</a>' +
...
function create_Team_Wordtable(data, _words) {
    ...
    var _teamcount = "";
    var table_body = '<thead><tr><th colspan = "4">搜尋[' +
        '<a group= "ALL"href="/mod/hsuforum/search.php?id=73&words=' + encodeURI(data[0][0]) + '&subject=組討論區" target="search_discuss" >' +
        data[0][0] +
        '</a>' +
