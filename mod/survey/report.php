<?php
    ...
    $id      = required_param('id', PARAM_INT);               $action  = optional_param('action', '', PARAM_ALPHA);     $qid     = optional_param('qid', 0, PARAM_RAW);           $student = optional_param('student', 0, PARAM_INT);       $notes   = optional_param('notes', '', PARAM_RAW);    
    $qids = explode(',', $qid);
    ...
    if ($groupmode = groups_get_activity_groupmode($cm)) {           $menuaction = $action == "student" ? "students" : $action;
    ...
    switch ($action) {
      ...
      case "scales":
        ...
        if (! $results = survey_get_responses($survey->id, $currentgroup, $groupingid) ) {
            ...
            foreach ($questionorder as $key => $val) {
                ...
                if ($question->type < 0) {                      $virtualscales = true;
                    break;
                }
            }
            ...
            foreach ($questionorder as $key => $val) {
                $question = $questions[$val];
                if ($question->multi) {
                    if (!empty($virtualscales) && $question->type > 0) {                          continue;
                    }
      ...
      case "questions":
        ...
        if ($qid) {                 $questions = $DB->get_records_select("survey_questions", "id in ($qid)");
        ...
        } else {                    $questions = $DB->get_records_list("survey_questions", "id", explode(',',$survey->questions));
            $questionorder = explode(",", $survey->questions);
      ...
      case "student":
         ...
         if ($showscales) {
             ...
             foreach ($questionorder as $key => $val) {
                 $question = $questions[$val];
                 if ($question->multi) {
                     if ($virtualscales && $question->type > 0) {                           continue;
                     }
                     echo "<p class=\"centerpara\">";
                 ...
                 if ($answer = survey_get_user_answer($survey->id, $question->id, $user->id)) {
                    ...
                    if (!empty($question->options) && $answer->answer1 > 0) {
                        ...
                        if ($answer->answer1 <= count($answers)) {
                            $table->data[] = array(s($answers[$answer->answer1 - 1]));                         } else {
