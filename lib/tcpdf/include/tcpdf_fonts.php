...
class TCPDF_FONTS {
	...
	public static function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false, $link=false) {
		...
		if (($fmetric['type'] == 'TrueType') OR ($fmetric['type'] == 'Type1')) {
			...
			$fmetric['AvgWidth'] = round($fmetric['AvgWidth'] / count($cwidths));
						...
						$offset = 0; 			if (TCPDF_STATIC::_getULONG($font, $offset) != 0x10000) {
								return false;
			}
			...
			$offset += 4;
			$offset += 2; 						$fmetric['unitsPerEm'] = TCPDF_STATIC::_getUSHORT($font, $offset);
			$offset += 2;
						$urk = (1000 / $fmetric['unitsPerEm']);
			$offset += 16; 			$xMin = round(TCPDF_STATIC::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			...
			if ($short_offset) {
								$tot_num_glyphs = floor($table['loca']['length'] / 2); 				for ($i = 0; $i < $tot_num_glyphs; ++$i) {
					...
					$offset += 2;
								...
								$tot_num_glyphs = floor($table['loca']['length'] / 4); 				for ($i = 0; $i < $tot_num_glyphs; ++$i) {
			...
			$offset += 2; 						$fmetric['AvgWidth'] = round(TCPDF_STATIC::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			...
			$offset += 2;
			$offset += 2; 			$fsType = TCPDF_STATIC::_getSHORT($font, $offset);
			$offset += 2;
			if ($fsType == 2) {
								return false;
			}
			...
			$offset += 2; 						$numNameRecords = TCPDF_STATIC::_getUSHORT($font, $offset);
			$offset += 2;
						$stringStorageOffset = TCPDF_STATIC::_getUSHORT($font, $offset);
			$offset += 2;
			for ($i = 0; $i < $numNameRecords; ++$i) {
				$offset += 6; 								$nameID = TCPDF_STATIC::_getUSHORT($font, $offset);
				$offset += 2;
				if ($nameID == 6) {
			...
			$offset += 4; 			$fmetric['italicAngle'] = TCPDF_STATIC::_getFIXED($font, $offset);
			$offset += 4;
			...
			$offset += 4; 						$fmetric['Ascent'] = round(TCPDF_STATIC::_getFWORD($font, $offset) * $urk);
			$offset += 2;
			...
			$offset += 2;
			$offset += 22; 						$numberOfHMetrics = TCPDF_STATIC::_getUSHORT($font, $offset);
						$offset = $table['maxp']['offset'];
			$offset += 4; 						$numGlyphs = TCPDF_STATIC::_getUSHORT($font, $offset);
						$ctg = array();
			foreach ($encodingTables as $enctable) {
					...
					$offset += 2;
					switch ($format) {
						case 0: { 							$offset += 4; 							for ($c = 0; $c < 256; ++$c) {
						...
						case 2: { 							$offset += 4; 							$numSubHeaders = 0;
							for ($i = 0; $i < 256; ++$i) {
						...
						case 4: { 							$length = TCPDF_STATIC::_getUSHORT($font, $offset);
							$offset += 2;
							$offset += 2; 							$segCount = floor(TCPDF_STATIC::_getUSHORT($font, $offset) / 2);
							$offset += 2;
							$offset += 6; 							$endCount = array(); 							for ($k = 0; $k < $segCount; ++$k) {
								$endCount[$k] = TCPDF_STATIC::_getUSHORT($font, $offset);
								$offset += 2;
							}
							$offset += 2; 							$startCount = array(); 							for ($k = 0; $k < $segCount; ++$k) {
								$startCount[$k] = TCPDF_STATIC::_getUSHORT($font, $offset);
								$offset += 2;
							}
							$idDelta = array(); 							for ($k = 0; $k < $segCount; ++$k) {
								$idDelta[$k] = TCPDF_STATIC::_getUSHORT($font, $offset);
								$offset += 2;
							}
							$idRangeOffset = array(); 							for ($k = 0; $k < $segCount; ++$k) {
								$idRangeOffset[$k] = TCPDF_STATIC::_getUSHORT($font, $offset);
								$offset += 2;
							}
							$gidlen = (floor($length / 2) - 8 - (4 * $segCount));
							$glyphIdArray = array(); 							for ($k = 0; $k < $gidlen; ++$k) {
						...
						case 6: { 							$offset += 4; 							$firstCode = TCPDF_STATIC::_getUSHORT($font, $offset);
							$offset += 2;
						...
						case 8: { 							$offset += 10; 							for ($k = 0; $k < 8192; ++$k) {
						...
						case 10: { 							$offset += 10; 							$startCharCode = TCPDF_STATIC::_getULONG($font, $offset);
							$offset += 4;
						...
						case 12: { 							$offset += 10; 							$nGroups = TCPDF_STATIC::_getULONG($font, $offset);
							$offset += 4;
							for ($k = 0; $k < $nGroups; ++$k) {
								$startCharCode = TCPDF_STATIC::_getULONG($font, $offset);
								$offset += 4;
		...
		if ($fmetric['type'] == 'Type1') {
			...
			if ($fmetric['type'] == 'cidfont0') {
				...
				$pfile .= '$ctg=\''.$fmetric['ctg'].'\';'."\n";
								$cidtogidmap = str_pad('', 131072, "\x00"); 				foreach ($ctg as $cid => $gid) {
	...
	public static function _getTrueTypeFontSubset($font, $subsetchars) {
		...
		if ($short_offset) {
				...
				$offset += 2;
						...
						$tot_num_glyphs = ($table['loca']['length'] / 4); 			for ($i = 0; $i < $tot_num_glyphs; ++$i) {
				$indexToLoc[$i] = TCPDF_STATIC::_getULONG($font, $offset);
				$offset += 4;
			}
		}
				$subsetglyphs = array(); 		$subsetglyphs[0] = true; 		$offset = $table['cmap']['offset'] + 2;
		$numEncodingTables = TCPDF_STATIC::_getUSHORT($font, $offset);
		$offset += 2;
		$encodingTables = array();
		for ($i = 0; $i < $numEncodingTables; ++$i) {
		...
		while (!empty($new_sga)) {
			...
			foreach ($sga as $key => $val) {
				if (isset($indexToLoc[$key])) {
					...
					$offset += 2;
					if ($numberOfContours < 0) { 						$offset += 8; 						do {
		...
		for ($i = 0; $i < $tot_num_glyphs; ++$i) {
			...
			$offset += $length;
						...
						$table_names = array ('head', 'hhea', 'hmtx', 'maxp', 'cvt ', 'fpgm', 'prep'); 				$offset = 12;
		...
		$table['glyf']['checkSum'] = self::_getTTFtableChecksum($table['glyf']['data'], $table['glyf']['length']);
				$font = '';
		$font .= pack('N', 0x10000); 		$numTables = count($table);
		$font .= pack('n', $numTables); 		$entrySelector = floor(log($numTables, 2));
		$searchRange = pow(2, $entrySelector) * 16;
		$rangeShift = ($numTables * 16) - $searchRange;
		$font .= pack('n', $searchRange); 		$font .= pack('n', $entrySelector); 		$font .= pack('n', $rangeShift); 		$offset = ($numTables * 16);
		foreach ($table as $tag => $data) {
			$font .= $tag; 			$font .= pack('N', $data['checkSum']); 			$font .= pack('N', ($data['offset'] + $offset)); 			$font .= pack('N', $data['length']); 		}
	...
	public static function arrUTF8ToUTF16BE($unicode, $setbom=false) {
		...
		foreach ($unicode as $char) {
			if ($char == 0x200b) {
				...
				$outstr .= "\xFF\xFD"; 			} elseif ($char < 0x10000) {
	...
	public static function UTF8ArrToLatin1Arr($unicode) {
		$outarr = array(); 		foreach ($unicode as $char) {
			if ($char < 256) {
				$outarr[] = $char;
	...
	public static function UTF8ArrToLatin1($unicode) {
		$outstr = ''; 		foreach ($unicode as $char) {
			if ($char < 256) {
				$outstr .= chr($char);
			} elseif (array_key_exists($char, TCPDF_FONT_DATA::$uni_utf8tolatin)) {
	...
	public static function getUniord($uch) {
		...
		$bytes = array(); 		$countbytes = 0;
		$numbytes = 1; 		$length = strlen($uch);
		for ($i = 0; $i < $length; ++$i) {
			$char = ord($uch[$i]); 			if ($countbytes == 0) { 				if ($char <= 0x7F) {
					return $char; 				} elseif (($char >> 0x05) == 0x06) { 					$bytes[] = ($char - 0xC0) << 0x06;
					++$countbytes;
					$numbytes = 2;
				} elseif (($char >> 0x04) == 0x0E) { 					$bytes[] = ($char - 0xE0) << 0x0C;
					++$countbytes;
					$numbytes = 3;
				} elseif (($char >> 0x03) == 0x1E) { 					$bytes[] = ($char - 0xF0) << 0x12;
					++$countbytes;
					$numbytes = 4;
				} else {
										return 0xFFFD;
				}
			} elseif (($char >> 0x06) == 0x02) { 				$bytes[] = $char - 0x80;
				++$countbytes;
	...
	public static function UTF8ToLatin1($str, $isunicode=true, &$currentfont) {
		$unicode = self::UTF8StringToArray($str, $isunicode, $currentfont); 		return self::UTF8ArrToLatin1($unicode);
	}
	...
	public static function UTF8ToUTF16BE($str, $setbom=false, $isunicode=true, &$currentfont) {
		if (!$isunicode) {
			return $str; 		}
		$unicode = self::UTF8StringToArray($str, $isunicode, $currentfont); 		return self::arrUTF8ToUTF16BE($unicode, $setbom);
	}
	...
	public static function utf8Bidi($ta, $str='', $forcertl=false, $isunicode=true, &$currentfont) {
		...
		$eor = $sor;
			...
			if ($ta[$i] == TCPDF_FONT_DATA::$uni_RLE) {
				...
				$prevlevel = -1; 		$levcount = 0; 		for ($i=0; $i < $numchars; ++$i) {
