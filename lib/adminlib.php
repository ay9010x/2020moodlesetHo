...
function uninstall_plugin($type, $name) {
    ...
    if ($type === 'mod') {
        $pluginname = $name;          if (get_string_manager()->string_exists('modulename', $component)) {
    ...
    if (file_exists($uninstalllib)) {
        ...
        $uninstallfunction = 'xmldb_' . $pluginname . '_uninstall';            if (function_exists($uninstallfunction)) {
                        $uninstallfunction();
        }
    }
    ...
    list($type, $name) = core_component::normalize_component($component);
        ...
        if ($source === 'installed') {
            ...
            if (!is_readable($CFG->dirroot.'/version.php')) {
                ...
                $version = null;                 include($CFG->dirroot.'/version.php');
                return $version;
            }
        }
    }
...
function is_dataroot_insecure($fetchtest=false) {
    ...
    foreach($rp as $r) {
        if (strpos($siteroot, '/'.$r.'/') === 0) {
            $siteroot = substr($siteroot, strlen($r)+1);         } else {
            break;         }
    }
...
abstract class admin_setting {
    ...
    public function config_write($name, $value) {
        ...
        $oldvalue = ($oldvalue === false) ? null : $oldvalue;         $value = is_null($value) ? null : (string)$value;
    ...
    public function get_defaultsetting() {
        ...
        if (!empty($adminroot->custom_defaults)) {
            ...
            if (isset($adminroot->custom_defaults[$plugin])) {
                if (array_key_exists($this->name, $adminroot->custom_defaults[$plugin])) {                     return $adminroot->custom_defaults[$plugin][$this->name];
                }
            }
        }
...
class admin_setting_configcheckbox extends admin_setting {
    ...
    public function write_setting($data) {
        if ((string)$data === $this->yes) {             $data = $this->yes;
        } else {
            $data = $this->no;
        }
        return ($this->config_write($this->name, $data) ? '' : get_string('errorsetting', 'admin'));
    ...
    public function output_html($data, $query='') {
        ...
        if ((string)$data === $this->yes) {             $checked = 'checked="checked"';
        } else {
            $checked = '';
        }
...
class admin_setting_configmultiselect extends admin_setting_configselect {
    ...
    public function output_html($data, $query='') {
        ...
        $return = '<div class="form-select"><input type="hidden" name="'.$this->get_full_name().'[xxxxx]" value="1" />';         $return .= '<select id="'.$this->get_id().'" name="'.$this->get_full_name().'[]" size="'.$size.'" multiple="multiple">';
        foreach ($this->choices as $key => $description) {
...
class admin_setting_special_calendar_weekend extends admin_setting {
    ...
    public function __construct() {
        ...
        $default = array ('0', '6');         parent::__construct($name, $visiblename, $description, $default);
    }
    ...
    public function output_html($data, $query='') {
        ...
        $return .= '<input type="hidden" name="'.$this->get_full_name().'[xxxxx]" value="1" />';         foreach($days as $index => $day) {
...
class admin_setting_special_coursecontact extends admin_setting_pickroles {
    ...
    public function __construct() {
        parent::__construct('coursecontact', get_string('coursecontact', 'admin'),
            get_string('coursecontact_desc', 'admin'), array('editingteacher'));
        $this->set_updatedcallback(create_function('', "cache::make('core', 'coursecontacts')->purge();"));
    }
}
...
class admin_setting_gradecat_combo extends admin_setting {
    ...
    public function get_setting() {
        ...
        $flag   = (int)$flag;
        $forced = (boolean)(1 & $flag);         $adv    = (boolean)(2 & $flag); 
...
function admin_externalpage_setup($section, $extrabutton = '', array $extraurlparams = null, $actualurl = '', array $options = array()) {
    ...
    $adminroot = admin_get_root(false, false);     $extpage = $adminroot->locate($section, true);
    ...
    if ($node instanceof admin_category) {
        ...
        foreach ($entries as $entry) {
            ...
            foreach ($node->settings as $setting) {
                if($setting->name == 'enablewebservices'){
                                    }
...
function db_replace($search, $replace) {
    ...
    if (!$tables = $DB->get_tables() ) {            return false;
    }
    foreach ($tables as $table) {
...
class admin_setting_enablemobileservice extends admin_setting_configcheckbox {
    ...
    public function output_html($data, $query='') {
        ...
        if ((string)$data === $this->yes) {
            ...
            $curl = new curl();
            $httpswwwroot = str_replace('http:', 'https:', $CFG->wwwroot);             $curl->head($httpswwwroot . "/login/index.php");
            $info = $curl->get_info();
    ...
    public function output_html($data, $query='') {
        ...
        $brtag = html_writer::empty_tag('br');
                ...
                get_string('configenablemobilewebservice',
                        'admin', ''), 0);         $manageserviceurl = new moodle_url("/admin/settings.php?section=mobile");
