...
class mod_choice_renderer extends plugin_renderer_base {
    ...
    public function display_result($choices, $forcepublish = false) {
        if (empty($forcepublish)) {             $forcepublish = $choices->publish;
        }

        $displaylayout = $choices->display;

        if ($forcepublish) {              return $this->display_publish_name_vertical($choices);
        } else {             if ($displaylayout == DISPLAY_HORIZONTAL_LAYOUT) {
