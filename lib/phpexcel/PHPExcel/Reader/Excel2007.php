...
class PHPExcel_Reader_Excel2007 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        $rels = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "_rels/.rels")), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());         foreach ($rels->Relationship as $rel) {
            if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
                $dir = dirname($rel["Target"]);
                $relsWorkbook = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "$dir/_rels/" . basename($rel["Target"]) . ".rels")), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());                  $relsWorkbook->registerXPathNamespace("rel", "http://schemas.openxmlformats.org/package/2006/relationships");
                ...
                $xmlWorkbook = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "{$rel['Target']}")), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());                  if ($xmlWorkbook->sheets) {
                    $dir = dirname($rel["Target"]);
                    foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                        $tmpInfo = array(
    ...
    public function load($pFilename)
    {
        ...
        if (!$this->readDataOnly) {
            $excel->removeCellStyleXfByIndex(0);             $excel->removeCellXfByIndex(0);         }
        ...
        $zip->open($pFilename);
                ...
                $wbRels = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "xl/_rels/workbook.xml.rels")), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());         foreach ($wbRels->Relationship as $rel) {
            switch ($rel["Type"]) {
            ...
            switch ($rel["Type"]) {
                case "http://schemas.openxmlformats.org/package/2006/relationships/metadata/core-properties":
                    ...
                    if (is_object($xmlCore)) {
                        ...
                        $docProps->setCreated(strtotime(self::getArrayItem($xmlCore->xpath("dcterms:created"))));                         $docProps->setModified(strtotime(self::getArrayItem($xmlCore->xpath("dcterms:modified"))));                         $docProps->setTitle((string) self::getArrayItem($xmlCore->xpath("dc:title")));
                ...
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
                    ...
                    $xmlStrings = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "$dir/$xpath[Target]")), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());                      if (isset($xmlStrings) && isset($xmlStrings->si)) {
                    ...
                    $sheetId = 0;                     $oldSheetId = -1;                     $countSkippedSheets = 0;                     $mapSheetId = array(); 
                    ...
                    if ($xmlWorkbook->sheets) {
                        foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                            ...
                            if (!$this->readDataOnly) {
                                    ...
                                    $relsWorksheet = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());                                     foreach ($relsWorksheet->Relationship as $ele) {
                            ...
                            if (!$this->readDataOnly) {
                                    ...
                                    if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
                                        $relsWorksheet = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());                                         $vmlRelationship = '';
                                        ...
                                        if ($vmlRelationship != '') {
                                                                                        $relsVML = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname($vmlRelationship) . '/_rels/' . basename($vmlRelationship) . '.rels')), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());                                             $drawings = array();
                                            ...
                                            $docSheet->getHeaderFooter()->setImages($hfImages);
                                                        ...
                                                        if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
                                $relsWorksheet = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());                                 $drawings = array();
                                foreach ($relsWorksheet->Relationship as $ele) {
                                ...
                                if ($xmlSheet->drawing && !$this->readDataOnly) {
                                    foreach ($xmlSheet->drawing as $drawing) {
                                        ...
                                        $relsDrawing = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname($fileDrawing) . "/_rels/" . basename($fileDrawing) . ".rels")), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());                                         $images = array();
                                    ...
                                    if (($spos = strpos($extractedRange, '!')) !== false) {
                                        ...
                                        continue;
                                            ...
                                            case '_xlnm.Print_Area':
                                                $rangeSets = explode(',', $extractedRange);                                                        $newRangeSets = array();
                                                foreach ($rangeSets as $rangeSet) {
                                                    $range = explode('!', $rangeSet);                                                        $rangeSet = isset($range[1]) ? $range[1] : $range[0];
