...
$cmid = required_param('cmid', PARAM_INT); $id = optional_param('id', 0, PARAM_INT); $edit = optional_param('edit', false, PARAM_BOOL); $assess = optional_param('assess', false, PARAM_BOOL); $delete = optional_param('delete', false, PARAM_BOOL); $confirm = optional_param('confirm', false, PARAM_BOOL); 
...
if ($id) {     $submission = $workshop->get_submission_by_id($id);
...
} else {     if (!$submission = $workshop->get_submission_by_author($USER->id)) {
...
if ($edit) {
    ...
    if ($mform->is_cancelled()) {
        ...
        $formdata->content            = '';                  $formdata->contentformat      = FORMAT_HTML;         $formdata->contenttrust       = 0;                   $formdata->late               = 0x0;                 if (!empty($workshop->submissionend) and ($workshop->submissionend < time())) {
