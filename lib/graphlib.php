...
class graph {
  var $image;
  var $debug             =   FALSE;          var $calculated        =   array();        var $parameter         =   array(            'width'              =>  320,              'height'             =>  240,              'file_name'          => 'none',                                                       'output_format'      => 'PNG',         
    'seconds_to_live'    =>  0,                'hours_to_live'      =>  0,                'path_to_fonts'      => 'fonts/',                                                 
    'title'              => 'Graph Title',     'title_font'         => 'default.ttf',       'title_size'         =>  16,               'title_colour'       => 'black',       
    'x_label'            => '',                'y_label_left'       => '',                'y_label_right'      => '',            
    'label_size'         =>  8,               'label_font'         => 'default.ttf',     'label_colour'       => 'gray33',          'y_label_angle'      =>  90,           
    'x_label_angle'      =>  90,            
    'outer_padding'      =>  5,                'inner_padding'      =>  0,                'x_inner_padding'      =>  5,                'y_inner_padding'      =>  6,                'outer_border'       => 'none',            'inner_border'       => 'black',           'inner_border_type'  => 'box',                                                                                                   'outer_background'   => 'none',            'inner_background'   => 'none',        
    'y_min_left'         =>  0,                'y_max_left'         =>  0,                'y_min_right'        =>  0,                'y_max_right'        =>  0,                'x_min'              =>  0,                'x_max'              =>  0,            
    'y_resolution_left'  =>  1,                                                                                                                                                                                                                                                                                  'y_decimal_left'     =>  0,                'y_resolution_right' =>  2,                'y_decimal_right'    =>  0,                'x_resolution'       =>  2,                'x_decimal'          =>  0,            
    'point_size'         =>  4,                'brush_size'         =>  4,                'brush_type'         => 'circle',                                                     'bar_size'           =>  0.8,                                                                                                    'bar_spacing'        =>  10,               'shadow_offset'      =>  3,                'shadow'             => 'grayCC',          'shadow_below_axis'  => true,         

    'x_axis_gridlines'   => 'auto',            'y_axis_gridlines'   =>  6,                'zero_axis'          => 'none',        

    'axis_font'          => 'default.ttf',     'axis_size'          =>  8,                'axis_colour'        => 'gray33',          'y_axis_angle'       =>  0,                'x_axis_angle'       =>  0,            
    'y_axis_text_left'   =>  1,                'x_axis_text'        =>  1,                'y_axis_text_right'  =>  0,            
    'x_offset'           =>  0.5,              'y_ticks_colour'     => 'black',           'x_ticks_colour'     => 'black',           'y_grid'             => 'line',            'x_grid'             => 'line',            'grid_colour'        => 'grayEE',          'tick_length'        =>  4,            
    'legend'             => 'none',                                                                                                'legend_offset'      =>  10,               'legend_padding'     =>  5,                'legend_font'        => 'default.ttf',       'legend_size'        =>  8,                'legend_colour'      => 'black',           'legend_border'      => 'none',        
    'decimal_point'      => '.',               'thousand_sep'       => ',',           
  );
  var $y_tick_labels     =   null;           var $offset_relation   =   null;         
    ...
    function draw_text() {
      ...
      if ($colour != 'none') $this->draw_rectangle($this->calculated['outer_border'], $colour, 'fill'); 
            $colour = $this->parameter['outer_border'];
      if ($colour != 'none') $this->draw_rectangle($this->calculated['outer_border'], $colour, 'box'); 
      $this->draw_title();
      $this->draw_x_label();
      ...
      $this->draw_x_axis();
      $this->draw_y_axis();
      if      ($this->calculated['y_axis_left']['has_data'])  $this->draw_zero_axis_left();        else if ($this->calculated['y_axis_right']['has_data']) $this->draw_zero_axis_right();       $this->draw_legend();

            $colour = $this->parameter['inner_background'];
      if ($colour != 'none') $this->draw_rectangle($this->calculated['inner_border'], $colour, 'fill'); 
            $colour = $this->parameter['inner_border'];
      if ($colour != 'none') $this->draw_rectangle($this->calculated['inner_border'], $colour, $this->parameter['inner_border_type']);     }

    function draw_stack() {
      $this->init();
      $this->draw_text();

      $yOrder = $this->y_order;             foreach ($yOrder as $set) {
    ...
    function draw() {
      ...
      foreach ($this->x_data as $index => $x) {
        ...
        if (($bar!='none') && (string)$thisY != 'none') {
            if ($relatedset = $this->offset_relation[$set]) {                                               $yoffset = $this->calculated['y_plot'][$relatedset][$index];                            } else {                                                                                        $yoffset = 0;                                                                           }                                                                                                       $this->bar($thisX, $thisY, $bar, $barSize, $colour, $offset, $set, $yoffset);           }
    ...
    function draw_legend() {
      ...
      switch ($position) {
        ...
        case 'outside-right' :
          ...
          $right  = $graphRight;
          break;
        default:           $top    = $this->calculated['boundary_box']['top'];
      ...
      $box = $this->calculated['legend']['boundary_box_max']['height'];       $x = $left + $padding;
      $x_text = $x + $box * 2;
      $y = $top + $padding;
    ...
    function draw_x_axis() {
      ...
      foreach ($this->calculated['x_axis']['tick_x'] as $set => $tickX) {
        ...
        if ($this->parameter['x_axis_text'] && !($set % $this->parameter['x_axis_text'])) {                     if ($tickColour != 'none')
    ...
    function draw_y_axis() {
              ...
              case 'line':
               ...
               case 'dash':
                ...
                break;
                    ...
                    if ($this->parameter['y_axis_text_left'] && !($set % $this->parameter['y_axis_text_left'])) {                         if ($tickColour != 'none')
      ...
      if ($this->calculated['y_axis_right']['has_data']) {
        ...
        foreach ($this->calculated['y_axis']['tick_y'] as $set => $tickY) {
          if (!$this->calculated['y_axis_left']['has_data'] && $yGrid != 'none') {             switch ($yGrid) {
              case 'line':
                ImageLine($this->image, round($gridLeft), round($tickY), round($gridRight), round($tickY), $gridColour);
                break;
               case 'dash':
          ...
          if ($this->parameter['y_axis_text_right'] && !($set % $this->parameter['y_axis_text_right'])) {                         if ($tickColour != 'none')
              ImageLine($this->image, round($tickLeft), round($tickY), round($tickRight), round($tickY), $tickColour);
    ...
    function init_data() {
      $this->calculated['y_plot'] = array();       $height = $this->calculated['boundary_box']['bottom'] - $this->calculated['boundary_box']['top'];
      $width  = $this->calculated['boundary_box']['right'] - $this->calculated['boundary_box']['left'];
            ...
            $extraTick  = 2 * $this->parameter['x_offset'];       $numTicks = $this->calculated['x_axis']['num_ticks'] - 1;    
      ...
      foreach ($this->y_order as $order => $set) {
          ...
          $this->calculated['bar_offset_index'][$set] = $this->calculated['num_bars'];           $this->calculated['num_bars']++;
        }
        ...
        $barX = ($barWidth - $totalWidth) / 2;         for ($i=0; $i < $this->calculated['num_bars']; $i++) {
    ...
    function init_x_ticks() {
                  $xStep       = $this->calculated['x_axis']['step'];
      $ticksOffset = $this->parameter['x_offset'];       $gridLeft    = $this->calculated['boundary_box']['left'] + ($xStep * $ticksOffset);       $tickX       = $gridLeft; 
    ...
    function init_legend() {
      $this->calculated['legend'] = array();             $this->calculated['legend']['boundary_box_max'] = $this->get_null_size();
      if ($this->parameter['legend'] == 'none') return;

      $position = $this->parameter['legend'];
      $numSets = 0;       $sumTextHeight = 0;       $width = 0;
      $height = 0;
      ...
      $offset  = $this->parameter['legend_offset'];        $padding = $this->parameter['legend_padding'];       $textWidth = $this->calculated['legend']['boundary_box_max']['width'];       $textHeight = $this->calculated['legend']['boundary_box_max']['height'];       $width = $padding * 2 + $textWidth + $textHeight * 2;        $height = ($padding + $textHeight) * $numSets + $padding; 
      $this->calculated['legend']['boundary_box_all'] = array('width'     => $width,
                                                            'height'    => $height,
                                                            'offset'    => $offset,
                                                            'reference' => $position);

      switch ($position) {         case 'outside-top' :
          $this->calculated['boundary_box']['right']      -= $offset + $width;           break;

        case 'outside-bottom' :
          $this->calculated['boundary_box']['right']      -= $offset + $width;           break;

        case 'outside-left' :
          $this->calculated['boundary_box']['bottom']      -= $offset + $height;           break;
    ...
    function init_y_axis() {
      $this->calculated['y_axis_left'] = array();       $this->calculated['y_axis_left']['boundary_box_max'] = $this->get_null_size();
      $this->calculated['y_axis_right'] = array();       $this->calculated['y_axis_right']['boundary_box_max'] = $this->get_null_size();
      ...
      if ($this->parameter['y_axis_text_left']) {
        for ($i = 0; $i < $this->parameter['y_axis_gridlines']; $i++) {                     if ($y_tick_labels) {
          ...
          $this->calculated['y_axis_left']['text'][$i]  = $value; 
          $size = $this->get_boundaryBox(
      ...
      if ($this->parameter['y_axis_text_right']) {
        for ($i = 0; $i < $this->parameter['y_axis_gridlines']; $i++) {                     $value = number_format($start_right, $this->parameter['y_decimal_right'], $this->parameter['decimal_point'], $this->parameter['thousand_sep']);
          $this->calculated['y_axis_right']['data'][$i]  = $start_right;
          $this->calculated['y_axis_right']['text'][$i]  = $value;           $size = $this->get_boundaryBox(
            array('points' => $axis_size,
                  'font'   => $axis_font,
                  'angle'  => $axis_angle,
                  'colour' => $axis_colour,
    ...
    function init_x_axis() {
      $this->calculated['x_axis'] = array();       $this->calculated['x_axis']['boundary_box_max'] = array('height' => 0, 'width' => 0);
      ...
      $axis_angle      = $this->parameter['x_axis_angle'];
            ...
            if ($this->parameter['x_axis_gridlines'] == 'auto') {         $this->calculated['x_axis']['num_ticks'] = sizeof($this->x_data);
          $data = $this->x_data;
          for ($i=0; $i < $this->calculated['x_axis']['num_ticks']; $i++) {
            $value = array_shift($data);             $this->calculated['x_axis']['data'][$i]  = $value;
            $this->calculated['x_axis']['text'][$i]  = $value;             $size = $this->get_boundaryBox(
              array('points' => $axis_size,
                    'font'   => $axis_font,
                    'angle'  => $axis_angle,
                    'colour' => $axis_colour,
      ...
      } else {         $this->calculated['x_axis']['num_ticks'] = $this->parameter['x_axis_gridlines'];
        ...
        for ($i = 0; $i < $this->calculated['x_axis']['num_ticks']; $i++) {           $value = number_format($start, $this->parameter['xDecimal'], $this->parameter['decimal_point'], $this->parameter['thousand_sep']);
          $this->calculated['x_axis']['data'][$i]  = $start;
          $this->calculated['x_axis']['text'][$i]  = $value; 
          $size = $this->get_boundaryBox(
            array('points' => $axis_size,
                  'font'   => $axis_font,
                  'angle'  => $axis_angle,
      ...
      if ($factor > 0.1) {         $factor = round($factor * 1000.0) / 1000.0;       } 
      ...
      switch ($reference) {
        ...
        default:
          ...
          break;
            ...
            $text = core_text::utf8_to_entities($text, true, true);             ImageTTFText($this->image, $points, $angle, $x, $y, $colour, $font, $text);
    }
    ...
    function get_boundaryBox($message) {
      ...
      if ($angle < 0) {
        ...
        $fontHeight = abs($bounds[7]-$bounds[1]);
                  ...
                  $text = core_text::utf8_to_entities($text, true, true);             $bounds = ImageTTFBBox($points, $angle, $font, $text);
    ...
    function draw_rectangle($border, $colour, $type) {
      ...
      switch ($type) {
        case 'fill':              ImageFilledRectangle($this->image, $border['left'], $border['top'], $border['right'], $border['bottom'], $colour);
          break;
        case 'box':               ImageRectangle($this->image, $border['left'], $border['top'], $border['right'], $border['bottom'], $colour);
          break;
        case 'axis':              ImageLine($this->image, $border['left'], $border['top'], $border['left'], $border['bottom'], $colour);
        ...
        case 'y':               case 'y-left':
          ...
          break;
        case 'y-right':           ImageLine($this->image, $border['right'], $border['top'], $border['right'], $border['bottom'], $colour);
          break;
        case 'x':                 ImageLine($this->image, $border['left'], $border['bottom'], $border['right'], $border['bottom'], $colour);
          break;
        case 'u':                 ImageLine($this->image, $border['left'], $border['top'], $border['left'], $border['bottom'], $colour);
    ...
    function init_colours() {
      $this->image              = ImageCreate($this->parameter['width'], $this->parameter['height']);
            $this->colour['white']    = ImageColorAllocate ($this->image, 0xFF, 0xFF, 0xFF);       $this->colour['black']    = ImageColorAllocate ($this->image, 0x00, 0x00, 0x00);
      ...
      if ($this->debug) {                                       } else {
        ...
        if ($this->parameter['file_name'] == 'none') {
          switch ($this->parameter['output_format']) {
            case 'GIF':
              Header("Content-type: image/gif");                ImageGIF($this->image);
              break;
            case 'JPEG':
    ...
    function bar($x, $y, $type, $size, $colour, $offset, $index, $yoffset) {
      ...
      if ($this->parameter['zero_axis'] != 'none') {
        ...
        if ($this->parameter['inner_border'] != 'none') $bottom -= 1;         $u_left  = $x_left + $offset;
        $u_right = $x_right + $offset - 1;
        $v       = $this->calculated['boundary_box']['bottom'] - $y + $offset;

                if ($yoffset) {                                                       $yoffset = $yoffset - round(($bottom - $v) / 2.0);                $bottom -= $yoffset;                                              $v      -= $yoffset;                                          }                                                         
        switch ($type) {
          case 'open':
    ...
    function area($x_start, $y_start, $x_end, $y_end, $type, $colour, $offset) {
        ...
        if ($this->parameter['shadow_below_axis'] ) $bottom  += $offset;
        if ($this->parameter['inner_border'] != 'none') $bottom -= 1;         switch ($type) {
          case 'fill':
