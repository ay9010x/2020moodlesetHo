...
function create_Persontable(data, _person) {
    ...
    for (var i = 0; i < number_of_rows; i++) {
        ...
        table_body += '</td>';
        table_body += '<td>';
        table_body += '<a word="' + data[i][2] + '" person="' + _person + '" count="' + data[i][3] + '" href="/mod/forum/search.php?id='+course+'&forumid='+forum+'&words=' + encodeURI(data[i][2]) + '&user=' + encodeURI(data[0][1]) + '" target="search_discuss111">' +
...
function create_Wordtable(data) {
    ...
    for (var i = 0; i < number_of_rows; i++) {
        ...
        table_body += '</td>';
        table_body += '<td>';
        table_body += '<a teamn=' + data[i][1] + ' user=' + data[i][2] + ' count=' + data[i][3] + ' href="/mod/forum/search.php?id='+course+'&forumid='+forum+'&words=' + encodeURI(data[i][1]) + '&user=' + data[i][2] + '" target="search_discuss111" >' +
            '<i class="circular search link icon"></i>' +
            '</a>';
        table_body += '</td>';
        table_body += '</tr>';
...
function create_Teamtable(data, _team) {
    ...
    var _ga_team = "";
    var _teamcount = "";
    var table_body = '<thead><tr><th colspan = "3">' + data[0][0] + '組' + '<button class="ui blue basic button" onclick="closeKeywordTable()" style="float: right;">關閉</button></th></tr><tr><th>詞彙</th><th>總次數</th><th>搜尋</th></tr></thead><tbody>';
    ...
    for (var i = 0; i < number_of_rows; i++) {
        ...
        table_body += '</td>';
        table_body += '<td>';
        table_body += '<a team="' + _team + '" word="' + data[i][1] + '" teamcount="' + data[i][2] + '" href="/mod/forum/search.php?id='+course+'&forumid='+forum+'&words=' + encodeURI(data[i][1]) + '&subject=' + encodeURI(_team) + '" target="search_discuss111">' +
            '<i class="circular search link icon"></i>' +
            '</a>';
        table_body += '</td>';
        table_body += '</tr>';
...
function create_Team_Wordtable(data, _words) {
    ...
    for (var i = 0; i < number_of_rows; i++) {
        ...
        if (_group !== _prev_group) {
            ...
            table_body += '</td>';
            table_body += '<td rowspan="' + _group_count[_group] + '">';
            table_body += '<a words="' + _words + '" group="' + data[i][1] + '" user="' + data[i][2] + '" teamcount="' + data[i][4] + '" href="/mod/forum/search.php?id='+course+'&forumid='+forum+'&words=' + encodeURI(_words) + '&subject=' + encodeURI(data[i][1]) + '組" target="search_discuss111">' +
